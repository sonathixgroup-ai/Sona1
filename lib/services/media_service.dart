...
// Vérifie que ce chemin correspond bien à ton projet :
...
class CommentItem { 
  ...
  factory CommentItem.fromMap(Map<String,dynamic> m) => CommentItem(
    ...
    // Utilisation ultra-sécurisée contre les bugs de type int/double :
    likeCount: (m['like_count'] as num?)?.toInt() ?? 0, 
    replyCount: (m['reply_count'] as num?)?.toInt() ?? 0
  );
}

class CommentPage { final List<CommentItem> items; final bool hasMore; final String? nextCursor; CommentPage({required this.items, required this.hasMore, this.nextCursor}); }
class FeedPage { final List<MediaContent> items; final double nextCursor; FeedPage({required this.items, required this.nextCursor}); }
class MediaCounts { final int likeCount, viewCount, commentCount; const MediaCounts({required this.likeCount, required this.viewCount, required this.commentCount}); }
...
class MediaService {
  ...
  static Future<void> _flushViews() async {
    ...
    final batch=_pendingViews.toList(); _pendingViews.clear(); _viewTimer=null;
    try{ await Supabase.instance.client.rpc('batch_register_views', params:{'p_media_ids': batch}); }catch(e){ _pendingViews.addAll(batch); }
  }
  ...
  Future<Set<String>> getLikedMediaIds(List<String> ids) async { if(ids.isEmpty) return {}; final r = await supabase.rpc('get_liked_media_ids', params:{'p_media_ids': ids}); return (r as List).map((e)=>e as String).toSet(); }
  ...
  Future<CommentPage> fetchRootComments(String mediaId,{int limit=20}) async {
    ...
    final hasMore = data.length>limit; final slice = hasMore? data.sublist(0,limit) : data;
    return CommentPage(items: slice.map((e)=>CommentItem.fromMap(e as Map<String,dynamic>)).toList(), hasMore: hasMore, nextCursor: slice.isNotEmpty? slice.last['created_at'] as String : null);
  }
  ...
  // =========================================================================
  // --- FONCTIONS MANQUANTES REQUISES PAR L'INTERFACE thix_media_page.dart ---
  // =========================================================================
  ...
  Future<CommentPage> fetchReplies(String parentId, {int limit=15}) async {
    ...
    return CommentPage(items: data.map((e) => CommentItem.fromMap(e as Map<String,dynamic>)).toList(), hasMore: false);
  }
  ...
  Future<void> toggleCommentLike(String commentId) async {
    try {
      ...
      // Ignorer l'erreur silencieusement côté front si la RPC n'existe pas encore
    }
  }
