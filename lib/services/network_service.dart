...
class NetworkService extends ChangeNotifier {
  ...
  Future<List<NetworkPost>> getFeedPosts({int limit = 20, int offset = 0, String feedType = 'smart'}) async {
    ...
    if (hiddenSet.isNotEmpty) {
      query = query.filter('id', 'not.in', '(${hiddenSet.join(',')})');
    }
  ...
  Future<NetworkPost?> getPostById(String postId) async {
    try {
      final res = await _supabase.from('posts_view').select().eq('id', postId).maybeSingle().timeout(const Duration(seconds: 10));
      return res == null ? null : NetworkPost.fromJson(res);
    } on TimeoutException {
      debugPrint('getPostById timeout');
      return null;
    } catch (e) {
  ...
  Future<NetworkCommunity> createCommunity({required String name, String? description, String? bannerUrl, String? logoUrl}) async {
    if (name.length > 100) throw Exception('Nom trop long');
    final res = await _supabase.from('communities').insert({
      'name': name.trim(), 'description': description?.trim(), 'logo_url': logoUrl ?? bannerUrl,
      'banner_url': bannerUrl, 'created_by': currentUserId, 'created_at': DateTime.now().toIso8601String(),
      'members_count': 1, 'posts_count': 0,
    }).select().single();
    await _supabase.from('community_members').upsert({
      'community_id': res['id'], 'user_id': currentUserId, 'role': 'admin', 'joined_at': DateTime.now().toIso8601String(),
  ...
  Future<String> createPost(String content, List<String> images) async {
    if (content.length > 5000) throw Exception('Contenu trop long (max 5000 caractères)');
    final res = await _supabase.from('posts').insert({
      'user_id': currentUserId, 'content': content.trim(), 'image_urls': images,
      'media_url': images.isNotEmpty ? images.first : null, 'is_public': true,
    }).select('id').single();
    notifyListeners();
  ...
  Future<String> createCommunityPost({required String communityId, required String content, List<String> images = const []}) async {
    if (content.length > 5000) throw Exception('Contenu trop long');
    final res = await _supabase.from('posts').insert({
      'user_id': currentUserId, 'community_id': communityId, 'content': content.trim(),
  ...
  Future<void> hidePost(String id) async {
    await _supabase.from('hidden_posts').upsert({
      'post_id': id, 'user_id': currentUserId, 'hidden_at': DateTime.now().toIso8601String()
    }, onConflict: 'post_id,user_id');
    notifyListeners();
  }
  Future<void> reportPost(String postId, String reason) async {
    await _supabase.from('reported_posts').insert({
      'post_id': postId, 'user_id': currentUserId, 'reason': reason, 'reported_at': DateTime.now().toIso8601String()
    });
  }
  ...
  Future<List<NetworkPost>> getSavedPosts() async {
    final res = await _supabase.from('saved_posts')
        .select('post:post_id(*, profiles:user_id(display_name, avatar_url, profession))')
        .eq('user_id', currentUserId).order('saved_at', ascending: false);
  ...
  Future<List<Comment>> getCommentsWithReplies(String postId, {int limit = 50, int offset = 0}) async {
    try {
      final res = await _supabase.from('comments')
          .select('*, profiles!user_id(display_name, avatar_url)')
          .eq('post_id', postId).is_('parent_id', null).order('created_at', ascending: true).range(offset, offset + limit - 1);
      return (res as List).map((j) => Comment.fromJson(j)).toList();
    } catch (e) { debugPrint('getCommentsWithReplies: $e'); return []; }
  }
  Future<List<Comment>> getCommentReplies(String parentId) async {
    final res = await _supabase.from('comments')
        .select('*, profiles!user_id(display_name, avatar_url)').eq('parent_id', parentId).order('created_at', ascending: true);
    return (res as List).map((j) => Comment.fromJson(j)).toList();
  }
  Future<Comment> addComment(String postId, String content, {String? parentId}) async {
    if (content.length > 1000) throw Exception('Commentaire trop long');
  ...
  Future<List<NetworkStory>> getActiveStories() async {
    try {
      final res = await _supabase.from('active_stories').select('*').order('created_at', ascending: false).limit(30);
  ...
  Future<void> createStory(String imageUrl, {int duration = 24}) async {
    await _supabase.from('stories').insert({'user_id': currentUserId, 'image_url': imageUrl, 'duration': duration.clamp(6, 48)});
    notifyListeners();
  }
  ...
  // ────────── HASHTAGS ──────────
  Future<HashtagResult> getPostsByHashtag(String tag, {int page = 0, int limit = 20}) async {
    final countRes = await _supabase.from('post_hashtags').select('post_id').eq('hashtag', tag).count(CountOption.exact);
    final total = countRes.count ?? 0;
    final res = await _supabase.from('post_hashtags')
        .select('post:post_id( ...posts_with_stats )')
        .eq('hashtag', tag).order('created_at', referencedTable: 'post').range(page * limit, (page + 1) * limit - 1);
    final posts = (res as List).map((e) => NetworkPost.fromJson(e['post'] as Map<String, dynamic>)).toList();
    return HashtagResult(posts, total);
  }

  // ────────── CONNEXIONS, MESSAGES, RECHERCHE ──────────
  ...
  Future<void> acceptConnectionRequest(String requestId) async {
    ...
    await _supabase.from('connections').upsert([{'user_id': req['sender_id'], 'connection_id': req['receiver_id'], 'status': 'accepted'}, {'user_id': req['receiver_id'], 'connection_id': req['sender_id'], 'status': 'accepted'}], onConflict: 'user_id,connection_id');
  }

  Future<List<NetworkCommunity>> getAllCommunities({int limit = 50}) async {
    try { final res = await _supabase.from('communities_with_membership').select().eq('current_user_id', currentUserId).order('members_count', ascending: false).limit(limit); return (res as List).map((e) => NetworkCommunity.fromJson(e)).toList(); } catch (_) { final res = await _supabase.from('communities').select().order('members_count', ascending: false).limit(limit); return (res as List).map((e) => NetworkCommunity.fromJson(e)).toList(); }
  }
  ...
  Future<int> getUnreadNotificationsCount() async {
    final res = await _supabase.from('notifications').select('id').eq('user_id', currentUserId).eq('is_read', false).count(CountOption.exact);
    return res.count ?? 0;
  }
  Future<int> getUnreadMessagesCount() async {
    final res = await _supabase.from('messages').select('id').eq('receiver_id', currentUserId).eq('is_read', false).count(CountOption.exact);
    return res.count ?? 0;
  }
...
class HashtagResult { final List<NetworkPost> posts; final int total; HashtagResult(this.posts, this.total); }
