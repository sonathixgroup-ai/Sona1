...
class NetworkService extends ChangeNotifier {
  ...
  // ─────────────────────────────────────────────
  // FEED - SCALABLE & OPTIMISÉ
  // ─────────────────────────────────────────────
  Future<List<NetworkPost>> getFeedPosts({
    int limit = 20,
    int offset = 0,
    String feedType = 'smart',
  }) async {
    ...
    // Récupérer les IDs cachés (optimisable avec NOT IN dans la requête)
    final hiddenRes = await _supabase
        .from('hidden_posts')
        .select('post_id')
        .eq('user_id', uid);
    final hiddenSet =
        (hiddenRes as List).map((e) => e['post_id'] as String).toSet();
    ...
    // Filtre anti‑posts cachés (si peu nombreux, ça reste efficace)
  ...
  Future<String> createPost(String content, List<String> images) async {
    if (content.length > 5000) {
      throw Exception('Contenu trop long (max 5000 caractères)');
    }
  ...
  Future<List<NetworkPost>> getSavedPosts() async {
    ...
    return (res as List)
        .map((e) => NetworkPost.fromJson(e['post'] as Map<String, dynamic>))
        .toList();
  }
  ...
  Future<List<Comment>> getCommentsWithReplies(
    ...
    try {
      final res = await _supabase
          ...
          .eq('post_id', postId)
          .is_('parent_id', null) // racines uniquement
  ...
  Future<List<Highlight>> getUserHighlights(String userId) async {
    final res = await _supabase
        ...
        .eq('user_id', userId)
        .order('created_at', ascending: false);
    return (res as List).map((e) => Highlight(
          id: e['id'],
          name: e['name'],
  ...
  Future<void> createHighlight(
      String name, List<String> storyIds, String? coverImage) async {
  ...
  // HASHTAGS - PERFORMANT
  // ─────────────────────────────────────────────
  Future<HashtagResult> getPostsByHashtag(String tag,
    ...
    // Récupérer les posts avec la vue agrégée
    final res = await _supabase
        .from('post_hashtags')
        .select('post:post_id( ...posts_with_stats )')
        .eq('hashtag', tag)
        .order('created_at', referencedTable: 'post')
        .range(page * limit, (page + 1) * limit - 1);

    final posts = (res as List).map((e) {
      final p = e['post'] as Map<String, dynamic>;
      return NetworkPost.fromJson(p);
    }).toList();
  ...
  // ─────────────────────────────────────────────
  // CONNEXIONS, MESSAGES, RECHERCHE, NOTIFICATIONS
  // ─────────────────────────────────────────────
  Future<List<NetworkConnection>> getSuggestedConnections(
      {int limit = 10}) async {
    try {
      final res = await _supabase.rpc('get_suggested_connections',
  ...
  Future<void> sendConnectionRequest(String targetId) async {
    ...
    if (targetId != currentUserId) {
      unawaited(
          _createNotification(userId: targetId, type: 'connection'));
    }
  }
    ...
    await _supabase.from('connections').upsert([
      {
        ...
        'connection_id': req['receiver_id'],
        'status': 'accepted'
      },
      {
        'user_id': req['receiver_id'],
        'connection_id': req['sender_id'],
        'status': 'accepted'
      },
  ...
  Future<List<NetworkCommunity>> getAllCommunities({int limit = 50}) async {
    try {
      final res = await _supabase
          ...
          .limit(limit);
      return (res as List)
          .map((e) => NetworkCommunity.fromJson(e))
          .toList();
    } catch (_) {
      final res = await _supabase
          .from('communities')
  ...
  Future<List<NetworkCommunity>> getMyCommunities() async {
    final res = await _supabase
        ...
        .eq('user_id', currentUserId);
    return (res as List)
        .map((e) => NetworkCommunity.fromJson({
              ...e['communities'],
              'is_member': true,
            }))
        .toList();
      ...
      for (var m in res as List) {
        ...
        if (!map.containsKey(otherId)) {
          ...
          map[otherId] = Conversation(
            ...
            unreadCount: (m['is_read'] == false && m['receiver_id'] == uid)
                ? 1
                : 0,
          );
        }
  ...
  Future<List<NetworkNotification>> getNotifications() async {
    final res = await _supabase
        ...
        .limit(50);
    return (res as List)
        .map((e) => NetworkNotification.fromJson(e))
        .toList();
  }
  ...
  Future<List<NetworkPost>> getUserPosts(String userId,
  ...
  Future<String?> uploadImageBytes(Uint8List bytes,
      {required String fileExtension, String bucket = 'post_images'}) async {
    try {
...
class HashtagResult {
  ...
  final int total;
  HashtagResult(this.posts, this.total);
}
