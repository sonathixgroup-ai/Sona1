...
class ChatMsgNotifier extends StateNotifier<List<ChatMessage>> {
  ...
  void upsertRealtime(List<ChatMessage> updated) {
    ...
    for (var msg in updated) {
      ...
      if (idx!= -1) {
        if (msg.isDeleted) current.removeAt(idx); else current[idx] = msg;
  ...
  void addLocal(ChatMessage msg) {
    if (!state.any((m) => m.id == msg.id)) state = [...state, msg];
  }
  void removeLocal(String id) => state = state.where((m) => m.id!= id).toList();
}
...
class _ChatScreenState extends ConsumerState<ChatScreen> with WidgetsBindingObserver {
  ...
  Future<void> _loadUserRole() async {
    ...
    if (user!= null) setState(() => _isAgent = user.role == 'agent' || user.role == 'admin' || user.role == 'support');
  }
  ...
  void _subscribeToPresence() {
    ...
    if (otherId.isNotEmpty) {
      svc.subscribeToPresence([otherId]).listen((list) { if (mounted && list.isNotEmpty) setState(() => _otherParticipant = list.first); });
    }
  }
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      final msg = await svc.sendMessage(conversationId: widget.conversationId, content: text, replyToId: _replyToId.isEmpty? null : _replyToId, isEphemeral: _isEphemeral, ephemeralDuration: _isEphemeral? _ephemeralDuration : null);
      ref.read(chatMessagesProvider(widget.conversationId).notifier).addLocal(msg);
      if (mounted) setState(() { _inputController.clear(); _replyToId = ''; if (_isInternalNoteMode) _isInternalNoteMode = false; });
      _scrollToBottom();
    } catch (e) {
  ...
  // ── UI dialogs identiques mais sans provider ──
  void _showEphemeralTimerDialog() { /*... garde ton code bottomSheet... */ }
  void _showPasswordProtectDialog() { /*... garde ton code... */ }
  void _showAttachmentMenu() { /*... */ }
  Future<void> _pickFile({FileType type = FileType.any}) async { /* utilise ref.read(chatServiceProvider) */ }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(children: [
          CircleAvatar(radius: 20, backgroundColor: _C.surfaceAlt, child: widget.conversation.isGroup? const Icon(Icons.groups_rounded, color: _C.textMuted) : ClipOval(child: CachedNetworkImage(imageUrl: widget.conversation.displayAvatar?? 'https://i.pravatar.cc/150?img=11', width: 40, height: 40, fit: BoxFit.cover))),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(widget.conversation.displayName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: _C.textMain), maxLines: 1, overflow: TextOverflow.ellipsis), if (!widget.conversation.isGroup && _otherParticipant!= null) Row(children: [Container(width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle, color: _otherParticipant!.status == 'online'? _C.green : _C.textMuted.withOpacity(0.5))), const SizedBox(width: 6), Text(_otherParticipant!.status == 'online'? 'En ligne' : 'Vu ${_otherParticipant!.lastSeenAt}', style: const TextStyle(fontSize: 12, color: _C.textMuted))])])),
        ]),
        actions: [
          ...
          PopupMenuButton<String>(icon: const Icon(Icons.more_vert_rounded, color: _C.textMain), color: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), onSelected: (v) { if (v == 'escalate') context.pushNamed('chatEscalate', pathParameters: {'conversationId': widget.conversationId}, queryParameters: {'agentId': ref.read(chatServiceProvider).currentUserId?? ''}); else if (v == 'history') context.pushNamed('chatEscalationHistory', pathParameters: {'conversationId': widget.conversationId}); }, itemBuilder: (c) => const [PopupMenuItem(value: 'escalate', child: Row(children: [Icon(Icons.arrow_upward, color: _C.orange, size: 20), SizedBox(width: 10), Text('Escalader')] )), PopupMenuItem(value: 'history', child: Row(children: [Icon(Icons.history, color: _C.primary, size: 20), SizedBox(width: 10), Text('Historique')]))]),
        ],
      ),
      body: SafeArea(
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                return ChatMessageBubble(message: msg, isOwn: isOwn, onReply: () => setState(() => _replyToId = msg.id), onDelete: () async { ref.read(chatMessagesProvider(widget.conversationId).notifier).removeLocal(msg.id); if (isOwn) { try { await ref.read(chatServiceProvider).deleteMessage(msg.id); } catch (_) {} } }, onReaction: (r) => ref.read(chatServiceProvider).toggleReaction(msg.id, r), replyToMessage: msg.replyToId!= null? messages.where((m) => m.id == msg.replyToId).firstOrNull : null, isEphemeralActive: msg.isEphemeral, isInternalNote: msg.isInternalNote, isAgentView: _isAgent);
              },
            ),
          ),
          if (_replyToId.isNotEmpty) Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), decoration: const BoxDecoration(color: Colors.white, border: Border(top: BorderSide(color: _C.border))), child: Row(children: [Container(width: 4, height: 40, decoration: BoxDecoration(color: _C.primary, borderRadius: BorderRadius.circular(4))), const SizedBox(width: 12), Expanded(child: Text(messages.firstWhere((m) => m.id == _replyToId, orElse: () => messages.first).content, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _C.textMuted))), IconButton(icon: const Icon(Icons.close_rounded, size: 20, color: _C.textMuted), onPressed: () => setState(() => _replyToId = ''))]),
          ),
          Container(color: Colors.white, padding: const EdgeInsets.only(top: 8), child: ChatInputBar(controller: _inputController, focusNode: _inputFocus, onSend: _sendMessage, isSending: false, onAttach: _showAttachmentMenu, onAudio: () {}, onSecureMessage: _showPasswordProtectDialog, onEphemeralToggle: _showEphemeralTimerDialog, isEphemeral: _isEphemeral, onTyping: (t) { if (t.isNotEmpty &&!_isTyping) { _isTyping = true; _sendTypingStatus(true); } else if (t.isEmpty && _isTyping) { _isTyping = false; _sendTypingStatus(false); } }, onInternalNoteToggle: _isAgent? () => setState(() => _isInternalNoteMode =!_isInternalNoteMode) : null, isInternalNote: _isInternalNoteMode)),
        ]),
      ),
    );
  }
