...
class _ChatListPageState extends State<ChatListPage> {
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(children: const [
                ...
                SizedBox(width: 8),
                Text('Notifications', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: _C.textDark)),
              ]),
            ),
            Flexible(
              child: _pendingEscalationsCount > 0
                  ? ListTile(
                      ...
                      title: Text('$_pendingEscalationsCount escalade(s) en attente', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                      onTap: () {
                        Navigator.pop(ctx);
                  ...
                  : const Padding(
                      ...
                      child: Center(child: Text('Aucune nouvelle notification', style: TextStyle(color: _C.textMuted, fontSize: 12.5))),
                    ),
            ),
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(
      ...
      : RefreshIndicator(onRefresh:_load, color:_C.blue, child: Stack(children:[
        Container(height: 240, decoration: const BoxDecoration(gradient: LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,colors:[_C.blue,_C.blueLight]))),
  ...
  // HEADER: THIX CHAT + search icône fonctionnelle + escalation VERTICAL + notif fonctionnelle
  Widget _headerTHIX(){
    return SafeArea(bottom:false, child: Padding(padding: const EdgeInsets.fromLTRB(16,12,12,10), child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(children:[
          const Text('THIX CHAT', style:TextStyle(fontSize:18, fontWeight:FontWeight.w900, color:Colors.white, letterSpacing:-.3)),
          const Spacer(),
          // search icône fonctionnelle
          InkWell(onTap:_toggleSearch, borderRadius:BorderRadius.circular(20), child: Container(width:30,height:30, decoration:BoxDecoration(color: _showSearchField ? Colors.white : Colors.white.withOpacity(.20), shape:BoxShape.circle, border:Border.all(color:Colors.white.withOpacity(.35),width:.8)), child:Icon(_showSearchField ? Icons.close_rounded : Icons.search_rounded, color: _showSearchField ? _C.blue : Colors.white, size:15))),
          ...
          const SizedBox(width:8),
          // notif fonctionnelle
          InkWell(onTap:_openNotifications, borderRadius:BorderRadius.circular(20), child: Container(width:32,height:32, decoration:BoxDecoration(color:Colors.white.withOpacity(.20),shape:BoxShape.circle,border:Border.all(color:Colors.white.withOpacity(.35),width:.8)), child:const Icon(Icons.notifications_none_rounded,color:Colors.white,size:16))),
        ]),
        if(_showSearchField) Padding(
          padding: const EdgeInsets.only(top: 14),
          child: Container(
            height: 38,
            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.black.withOpacity(.06), blurRadius: 10)]),
            child: TextField(
              controller: _searchCtrl,
              autofocus: true,
              onChanged: _onSearch,
              style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: _C.textDark),
              decoration: const InputDecoration(
                hintText: 'Rechercher un chat...',
                hintStyle: TextStyle(fontSize: 11.5, color: _C.textMuted),
                prefixIcon: Icon(Icons.search_rounded, size: 16, color: _C.blue),
