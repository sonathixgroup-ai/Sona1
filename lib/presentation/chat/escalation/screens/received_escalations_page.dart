...
class ReceivedEscalationsPage extends ConsumerStatefulWidget {
  ...
  @override 
  ConsumerState<ReceivedEscalationsPage> createState() => _ReceivedEscalationsPageState();
}
...
class _ReceivedEscalationsPageState extends ConsumerState<ReceivedEscalationsPage> {
  ...
  void _loadData({required bool refresh}) {
    ...
    if (user != null) {
      final level = ref.read(escalationServiceProvider).getCurrentLevelForAgent(user.id);
      ref.read(escalationProvider.notifier).loadPending(user.id, level, refresh: refresh);
    }
  }
  ...
  Future<void> _openConversation(String conversationId) async {
    try {
      ...
      if (conv != null && mounted) {
        Navigator.push(
          context, 
          MaterialPageRoute(builder: (_) => ChatScreen(conversationId: conversationId, conversation: conv))
        );
        return;
      }
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Conversation introuvable.'), backgroundColor: _C.textMain)
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _accept(String id) async {
    ...
    if (ok != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Escalade acceptée'), backgroundColor: Color(0xFF16A34A))
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Erreur: ${ref.read(escalationProvider).error}'), backgroundColor: _C.red)
      );
    }
  }
  ...
  Future<void> _reject(String id) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Refuser l\'escalade', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: _C.textMain)),
        content: TextField(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('Refuser', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))
          ),
        ],
      ),
    );
    ...
    if (ok != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Escalade refusée'), backgroundColor: _C.textMain)
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Erreur: ${ref.read(escalationProvider).error}'), backgroundColor: _C.red)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Escalades reçues', style: TextStyle(color: _C.textMain, fontSize: 16, fontWeight: FontWeight.bold)),
        actions: [
          IconButton(
  ...
  Widget _buildBody(EscalationState state) {
    if (state.isLoading && state.pending.isEmpty) {
      return const Center(child: CircularProgressIndicator(color: _C.primary, strokeWidth: 3));
    }
    
    if (state.error != null && state.pending.isEmpty) {
      return Center(
        child: Text('Erreur : ${state.error}', style: const TextStyle(color: _C.textMuted, fontSize: 14))
      );
    }
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: const BoxDecoration(color: _C.searchBg, shape: BoxShape.circle),
              child: const Icon(Icons.inbox_outlined, size: 48, color: _C.textMuted)
            ), 
            const SizedBox(height: 16), 
            const Text('Aucune escalade reçue', style: TextStyle(color: _C.textMain, fontSize: 16, fontWeight: FontWeight.bold))
          ]
        )
      );
    }
        ...
        itemBuilder: (context, index) {
          if (index == state.pending.length) {
            return const Padding(
              ...
              child: Center(child: SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 3, color: _C.primary)))
            );
          }
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.person_outline_rounded, size: 16, color: _C.textMuted)
                        ),
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(color: step.status.color.withOpacity(0.2))
                          ), 
                          child: Text(
                            step.status.label, 
                            style: TextStyle(color: step.status.color, fontSize: 11, fontWeight: FontWeight.bold)
                          )
                        ),
                      ]
                    ),
                    ...
                    Text('Raison : ${step.reason}', style: const TextStyle(fontSize: 14, color: _C.textMain, fontWeight: FontWeight.w500)),
                    
                    if (step.comment != null) 
                      Padding(
                        padding: const EdgeInsets.only(top: 8), 
                        child: Text('Commentaire : ${step.comment}', style: const TextStyle(fontSize: 13, color: _C.textMuted, fontStyle: FontStyle.italic))
                      ),
                      
                    const SizedBox(height: 12),
                    Text('Demandé le ${step.createdAt.toString().substring(0, 16)}', style: const TextStyle(fontSize: 12, color: _C.textMuted)),
