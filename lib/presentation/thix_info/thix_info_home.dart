...
class _PatientDashboardPageState extends ConsumerState<PatientDashboardPage> {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        ...
        slivers: [
          ...
          SliverToBoxAdapter(child: _grid(_getRapidesItems(), spacing: 6)), // Espacement réduit
          SliverToBoxAdapter(child: _sectionHeader('Parcours Santé', Icons.local_hospital_rounded)),
          SliverToBoxAdapter(child: _grid(_getSanteItems(), spacing: 6)), // Espacement réduit
  ...
  List<Map<String,dynamic>> _getRapidesItems() => [
    ...
    {'l':'Téléconsult.','i':Icons.videocam,'p':const TeleconsultationPage()},
    {'l':'Assistant IA','i':Icons.smart_toy,'p':const AssistantIAPage()},
    {'l':'Dossier part.','i':Icons.share,'p':const DossierPartagePage()},
    {'l':'Épidémies','i':Icons.coronavirus,'p':const EpidemiesPage()},
    {'l':'Don sang','i':Icons.water_drop,'p':const DonSangPage()},
    {'l':'Vaccins','i':Icons.vaccines,'p':const RappelsVaccinPage()},
    {'l':'Certificat','i':Icons.description,'p':const CertificatMedicalPage()},
    {'l':'Assurance','i':Icons.shield,'p':const AssuranceSantePage()},
  ];
  ...
  List<Map<String,dynamic>> _getSanteItems() => [
    ...
    {'l':'Analyse','i':Icons.show_chart,'p':const AnalysePredictivePage()},
    {'l':'Mental','i':Icons.psychology,'p':const BienEtreMentalPage()},
    {'l':'Nutrition','i':Icons.apple,'p':const NutritionPage()},
    {'l':'Physique','i':Icons.fitness_center,'p':const ActivitePhysiquePage()},
    {'l':'Stress','i':Icons.self_improvement,'p':const GestionStressPage()},
  ];
  ...
  Widget _sosBanner() => Container(
    ...
    child: Row(children:[
      ...
      IconButton(icon: const Icon(Icons.call, color: Colors.white, size: 18), onPressed: ()=>_go(const UrgencesProchesPage()))
    ])
  );
