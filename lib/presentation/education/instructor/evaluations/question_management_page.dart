...
class _QuestionManagementPageState extends State<QuestionManagementPage> {
  // Liste des questions (stockées en local)
  ...
  // Ajoute la question courante à la liste
  void _addQuestion() {
  ...
  // Sauvegarde les questions dans Supabase (ou via provider)
  Future<void> _saveQuestions() async {
    setState(() => _isLoading = true);
    try {
      // TODO: Appeler le service pour sauvegarder les questions
      // final provider = context.read<EducationProvider>();
      // await provider.saveQuestions(widget.evaluationId, _questions);

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Questions sauvegardées !')),
      );
