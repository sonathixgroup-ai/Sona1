...
class ModuleManagementPage extends ConsumerStatefulWidget {
  final Module? module;
  final String? formationId; // Requis si on crée un module pour une formation précise

  const ModuleManagementPage({super.key, this.module, this.formationId});
...
class _ModuleManagementPageState extends ConsumerState<ModuleManagementPage> {
  ...
  Future<void> _addLesson() async {
    final newLesson = await Navigator.push<Lesson>(
      context,
      MaterialPageRoute(builder: (_) => const LessonManagementPage()),
    );
    if (newLesson != null) {
  ...
  Future<void> _saveModule() async {
    ...
    try {
      final moduleData = {
        ...
        if (widget.formationId != null) 'formation_id': widget.formationId,
      ...
      // Retourner le module construit vers la page parente
      final resultModule = Module(
        id: moduleId,
        formationId: widget.formationId ?? widget.module?.formationId ?? '',
        title: _titleController.text.trim(),
        description: _descriptionController.text.trim(),
        order: widget.module?.order ?? 0,
        lessons: _lessons,
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Détails du module', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: _C.textMain)),
                    ...
                    TextFormField(
                      ...
                      maxLines: 3,
                      style: const TextStyle(fontWeight: FontWeight.w500),
                      decoration: _inputDecoration('Description du module', Icons.description_outlined),
                    ),
                  ],
                ),
              ),
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Container(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Text('Type : ${lesson.type ?? 'Standard'}', style: const TextStyle(color: _C.textMuted, fontSize: 12)),
                              trailing: Row(
