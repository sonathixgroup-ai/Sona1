...
class _PostCardState extends ConsumerState<PostCard> with AutomaticKeepAliveClientMixin {
  ...
  List<InlineSpan> _parseContent(String content, TextStyle baseStyle) {
    ...
    for (final match in _richContentRegex.allMatches(content)) {
      ...
      if (match.group(1)!= null) {
        ...
        spans.add(TextSpan(children: _parseContent(match.group(3)!, baseStyle.copyWith(fontWeight: FontWeight.w800))));
      } else if (match.group(4)!= null) {
        spans.add(TextSpan(children: _parseContent(match.group(4)!, baseStyle.copyWith(fontStyle: FontStyle.italic))));
      } else if (match.group(5)!= null) {
        final value = match.group(5)!;
        final recognizer = TapGestureRecognizer()..onTap = () => context.push('/profile/$value');
        ...
        final recognizer = TapGestureRecognizer()..onTap = () => context.push('/hashtag/$value');
        _recognizers.add(recognizer);
  ...
  void _openGallery(int initialIndex, List<String> urls, String postId) {
    Navigator.push(context, PageRouteBuilder(
      opaque: false, barrierColor: Colors.black, transitionDuration: const Duration(milliseconds: 220),
  ...
  Widget _buildNetworkImage(String url, {double? width, double height = 200, BoxFit fit = BoxFit.cover, Alignment alignment = Alignment.center}) {
    return CachedNetworkImage(
      imageUrl: url, width: width, height: height, fit: fit, alignment: alignment,
  ...
  Widget _tappableImage(String url, int index, List<String> allUrls, String postId, {required double height}) {
    return GestureDetector(
      ...
      child: Hero(tag: 'post_${postId}_image_$index', child: _buildNetworkImage(url, width: double.infinity, height: height, alignment: Alignment.topCenter)),
    );
  }
  ...
  Widget _buildImageGrid(List<String> urls, String postId) {
    ...
    if (urls.length == 1) {
      return ClipRRect(borderRadius: BorderRadius.circular(18), child: GestureDetector(onTap: () => _openGallery(0, urls, postId), child: Hero(tag: 'post_${postId}_image_0', child: _AdaptiveSingleImage(imageUrl: urls[0]))));
    }
    if (urls.length == 2) {
      return Row(children: [Expanded(child: ClipRRect(borderRadius: radius, child: GestureDetector(onTap: () => _openGallery(0, urls, postId), child: Hero(tag: 'post_${postId}_image_0', child: _AdaptivePairImage(imageUrl: urls[0], groupKey: urls))))), const SizedBox(width: spacing), Expanded(child: ClipRRect(borderRadius: radius, child: GestureDetector(onTap: () => _openGallery(1, urls, postId), child: Hero(tag: 'post_${postId}_image_1', child: _AdaptivePairImage(imageUrl: urls[1], groupKey: urls))))),]);
    }
    if (urls.length == 3) {
      return SizedBox(height: 240, child: Row(children: [Expanded(flex: 3, child: ClipRRect(borderRadius: radius, child: _tappableImage(urls[0], 0, urls, postId, height: 240))), const SizedBox(width: spacing), Expanded(flex: 2, child: Column(children: [Expanded(child: ClipRRect(borderRadius: radius, child: _tappableImage(urls[1], 1, urls, postId, height: 118))), const SizedBox(height: spacing), Expanded(child: ClipRRect(borderRadius: radius, child: _tappableImage(urls[2], 2, urls, postId, height: 118)))]))]));
    }
    if (urls.length == 4) {
      return SizedBox(height: 320, child: Column(children: [Expanded(child: Row(children: [Expanded(child: ClipRRect(borderRadius: radius, child: _tappableImage(urls[0], 0, urls, postId, height: 158))), const SizedBox(width: spacing), Expanded(child: ClipRRect(borderRadius: radius, child: _tappableImage(urls[1], 1, urls, postId, height: 158)))])), const SizedBox(height: spacing), Expanded(child: Row(children: [Expanded(child: ClipRRect(borderRadius: radius, child: _tappableImage(urls[2], 2, urls, postId, height: 158))), const SizedBox(width: spacing), Expanded(child: ClipRRect(borderRadius: radius, child: _tappableImage(urls[3], 3, urls, postId, height: 158)))]))]));
    }
  ...
  Widget _buildPollWidget(NetworkPost post, WidgetRef ref) {
    ...
    int totalVotes = 0;
    for (var opt in options) totalVotes += ((opt['votes'] as List?)?.length?? 0);
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: options.asMap().entries.map((entry) {
      ...
      final double percentage = totalVotes > 0? voters.length / totalVotes : 0.0;
  ...
  Widget _buildChallengeWidget(NetworkPost post) {
    final challengeData = post.challengeData?? {};
  ...
  Widget build(BuildContext context) {
    ...
    return ProviderScope(
      ...
      child: Consumer(builder: (context, ref, _) {
        ...
        final likesCount = ref.watch(postItemProvider(widget.post.id).select((p) => p.likesCount));
          ...
          Row(children: [
            ...
            PopupMenuButton<String>(icon: const Icon(Icons.more_vert_rounded, size: 18, color: _PostColors.textSecondary), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), onSelected: (v) async {
              switch(v){
                ...
                case 'save': await ref.read(postItemProvider(post.id).notifier).toggleSave(); widget.onSave?.call(); break;
            ...
            }, itemBuilder: (_) => [if(isOwner)...[PopupMenuItem(value: 'edit', child: Row(children: [const Icon(Icons.edit_rounded, size: 18, color: _PostColors.primary), const SizedBox(width: 8), const Text('Modifier')])), PopupMenuItem(value: 'pin', child: Row(children: [const Icon(Icons.push_pin_rounded, size: 18, color: _PostColors.gold), const SizedBox(width: 8), const Text('Épingler')])), PopupMenuItem(value: 'delete', child: Row(children: [const Icon(Icons.delete_rounded, size: 18, color: _PostColors.red), const SizedBox(width: 8), const Text('Supprimer', style: TextStyle(color: _PostColors.red))]))], const PopupMenuItem(value: 'save', child: Row(children: [Icon(Icons.bookmark_border_rounded, size: 18, color: _PostColors.primaryDeep), SizedBox(width: 8), Text('Sauvegarder')])), const PopupMenuItem(value: 'repost', child: Row(children: [Icon(Icons.repeat_rounded, size: 18), SizedBox(width: 8), Text('Reposter')])), const PopupMenuItem(value: 'hide', child: Row(children: [Icon(Icons.visibility_off_rounded, size: 18), SizedBox(width: 8), Text('Masquer')])), const PopupMenuItem(value: 'report', child: Row(children: [Icon(Icons.flag_rounded, size: 18, color: Colors.orange), SizedBox(width: 8), Text('Signaler')])), const PopupMenuItem(value: 'share', child: Row(children: [Icon(Icons.share_rounded, size: 18), SizedBox(width: 8), Text('Partager')]))]),
          ]),
  ...
  Future<void> _repost(NetworkPost post, WidgetRef ref) async {
    ...
    try { await ref.read(networkServiceProvider).repost(post.id, _quoteController.text); ref.read(postItemProvider(post.id).notifier).incRepost(); if(mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Post reposté'), backgroundColor: _PostColors.green)); widget.onRefresh?.call(); } catch(e){ if(mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erreur: $e'), backgroundColor: _PostColors.red)); } finally { if(mounted) setState(()=>_isReposting=false); _quoteController.clear(); }
  }
...
class _AdaptiveSingleImage extends StatefulWidget {
  ...
  @override State<_AdaptiveSingleImage> createState() => _AdaptiveSingleImageState();
}
class _AdaptiveSingleImageState extends State<_AdaptiveSingleImage> {
  static const _minHeight = 220.0;
...
class _AdaptivePairImage extends StatefulWidget {
  final String imageUrl; final List<String> groupKey;
  const _AdaptivePairImage({required this.imageUrl, required this.groupKey});
  @override State<_AdaptivePairImage> createState() => _AdaptivePairImageState();
}
class _AdaptivePairImageState extends State<_AdaptivePairImage> {
  static const _minHeight=180.0; static const _maxHeight=320.0;
  double? _aspectRatio; ImageStream? _stream; late ImageStreamListener _listener;
...
class _FullScreenGallery extends StatefulWidget {
  final List<String> imageUrls; final int initialIndex; final String postId;
  const _FullScreenGallery({required this.imageUrls, required this.initialIndex, required this.postId});
