...
class _NetworkProHomeState extends ConsumerState<NetworkProHome> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    if (currentUser == null) {
      return const Scaffold(backgroundColor: ThixColors.background, body: Center(child: CircularProgressIndicator()));
    }
            ...
            child: CustomScrollView(
              ...
              slivers: [
                ...
                feedAsync.when(
                  ...
                  error: (e, _) => SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(40), child: Center(child: Text('Erreur: $e')))),
                  data: (posts) {
  ...
  Widget _buildSliverAppBar() {
    return SliverAppBar(
      backgroundColor: ThixColors.white, elevation: 0, scrolledUnderElevation: 1, floating: true, snap: true, toolbarHeight: 50, titleSpacing: 16,
      title: ShaderMask(
      ...
      actions: [
        IconButton(icon: const Icon(Icons.search_rounded, size: 21, color: ThixColors.textDark), onPressed: () => context.push('/network/search')),
        IconButton(icon: const Badge(label: Text('3'), child: Icon(Icons.notifications_none_rounded, size: 21, color: ThixColors.textDark)), onPressed: () => context.push('/network/notifications')),
        const SizedBox(width: 2),
        Padding(padding: const EdgeInsets.only(right: 12), child: GestureDetector(onTap: () => context.push('/profile'), child: const CircleAvatar(radius: 14, backgroundColor: ThixColors.softBlue, child: Icon(Icons.person, size: 15)))),
      ],
    );
  }
  ...
  Widget _buildStories(String currentUserId) {
    ...
    return Container(
      color: ThixColors.white, height: 88, padding: const EdgeInsets.only(top: 10, bottom: 8),
      child: ListView.separated(
        scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 14), itemCount: otherStories.length + 1,
        separatorBuilder: (_, __) => const SizedBox(width: 12),
        itemBuilder: (c, i) {
          if (i == 0) {
            return _MyStoryItem(hasStory: myStories.isNotEmpty, avatarUrl: myStories.isNotEmpty ? myStories.first.userAvatar : null, onView: myStories.isNotEmpty ? () => Navigator.push(context, MaterialPageRoute(builder: (_) => StoryViewer(stories: myStories, initialIndex: 0))) : _openCreateStory, onAdd: _openCreateStory);
          }
  ...
  Widget _buildFilters() {
    ...
    return Container(
      color: ThixColors.white, padding: const EdgeInsets.fromLTRB(14, 0, 14, 12),
  ...
  Widget _buildCreatePostBar() {
    return Container(margin: const EdgeInsets.all(14), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: ThixColors.border), boxShadow: [BoxShadow(color: ThixColors.shadow, blurRadius: 12, offset: const Offset(0, 4))]),
      child: Row(children: [
        const CircleAvatar(radius: 18, backgroundColor: ThixColors.softBlue, child: Icon(Icons.person, size: 18)),
        const SizedBox(width: 10),
        Expanded(child: GestureDetector(onTap: () => showDialog(context: context, builder: (_) => const CreatePostDialog()).then((v) { if (v == true) ref.read(feedProvider.notifier).loadFeed(force: true); }),
  ...
  Widget _buildSuggestions() {
    return Container(margin: const EdgeInsets.all(14), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: ThixColors.border)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ..._suggestions.map((u) => ListTile(dense: true, contentPadding: EdgeInsets.zero, leading: CircleAvatar(backgroundImage: u.avatar != null ? NetworkImage(u.avatar!) : null), title: Text(u.name, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)), subtitle: Text(u.title ?? '', style: const TextStyle(fontSize: 11)), trailing: FilledButton.tonal(onPressed: () async { await ref.read(networkServiceProvider).sendConnectionRequest(u.id); setState(() => _suggestions.remove(u)); }, child: const Text('Suivre')))),
      ]),
    );
  }
...
class _MyStoryItem extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    return GestureDetector(onTap: onView, child: Column(children: [
      Stack(clipBehavior: Clip.none, children: [
        hasStory ? Container(padding: const EdgeInsets.all(2.5), decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [ThixColors.primary, ThixColors.gold])), child: CircleAvatar(radius: 23, backgroundColor: ThixColors.softBlue, backgroundImage: avatarUrl != null && avatarUrl!.isNotEmpty ? NetworkImage(avatarUrl!) : null, child: avatarUrl == null || avatarUrl!.isEmpty ? const Icon(Icons.person, size: 22, color: ThixColors.primaryDeep) : null))
            : Container(width: 52, height: 52, decoration: BoxDecoration(shape: BoxShape.circle, color: ThixColors.softBlue, border: Border.all(color: ThixColors.border, width: 1.5)), child: const Icon(Icons.person, size: 24, color: ThixColors.primaryDeep)),
        Positioned(bottom: -2, right: -2, child: GestureDetector(onTap: onAdd, child: Container(width: 20, height: 20, decoration: BoxDecoration(shape: BoxShape.circle, color: ThixColors.gold, border: Border.all(color: ThixColors.white, width: 2)), child: const Icon(Icons.add_rounded, size: 13, color: Colors.white)))),
      ]),
      const SizedBox(height: 4),
      Text(hasStory ? 'Votre story' : 'Ajouter', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: ThixColors.textDark)),
    ]));
  }
}
...
class _StoryItem extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    return GestureDetector(onTap: onTap, child: Column(children: [
      Container(padding: const EdgeInsets.all(2.5), decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [ThixColors.primary, ThixColors.gold])), child: CircleAvatar(radius: 23, backgroundImage: story.userAvatar != null ? NetworkImage(story.userAvatar!) : null)),
      const SizedBox(height: 4),
      SizedBox(width: 56, child: Text(story.userName.split(' ').first, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600))),
    ]));
  }
}
