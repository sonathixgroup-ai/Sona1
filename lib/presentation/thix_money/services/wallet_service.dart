...
class WalletModel {
  ...
  final int soldeCdf;
  final int soldeUsd;
  WalletModel({required this.thixId, required this.soldeCdf, required this.soldeUsd});
...
class WalletService {
  ...
  Stream<WalletModel> streamWallet() async* {
    final thixId = await getVerifiedThixId();
    yield* _db.from('thix_wallets').stream(primaryKey: ['id']).eq('thix_id', thixId).map((list) {
      if (list.isEmpty) return WalletModel(thixId: thixId, soldeCdf: 0, soldeUsd: 0);
