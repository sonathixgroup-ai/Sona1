...
import '../supermarket/supermarket_detail.dart';
...
class _MarketHomePageState extends State<MarketHomePage> {
  ...
  Widget _buildSectionHeader(String title, VoidCallback onTap) { return Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: _MarketColors.darkText)), GestureDetector(onTap: onTap, child: const Row(children: [Text('Voir tout', style: TextStyle(color: _MarketColors.red, fontSize: 12, fontWeight: FontWeight.w800)), SizedBox(width: 4), Icon(Icons.arrow_forward_ios_rounded, size: 10, color: _MarketColors.red)]))])); }
  
  Widget _buildProductHorizontalCard(Map<String, dynamic> product, {bool isFlash = false}) { final currency = product['currency'] ?? 'FC'; final symbol = currency == 'USD' ? '\$' : 'FC'; final price = (product['price'] as num).toInt(); return GestureDetector(onTap: () => context.push('/market/product/${product['id']}'), child: Container(width: 155, decoration: BoxDecoration(color: _MarketColors.pureWhite, borderRadius: BorderRadius.circular(16), border: Border.all(color: isFlash ? _MarketColors.red.withOpacity(0.25) : _MarketColors.cardBorder, width: 1), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 10, offset: const Offset(0, 4))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(flex: 5, child: Stack(fit: StackFit.expand, children: [ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(16)), child: _networkImage(product['image_url'])), if (isFlash) Positioned(top: 8, left: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: _MarketColors.red, borderRadius: BorderRadius.circular(8)), child: Text('-${product['discount_percent'] ?? 0}%', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w900)))), Positioned(top: 4, right: 4, child: Container(padding: const EdgeInsets.all(2), decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: WishlistButton(productId: product['id'].toString(), size: 20, activeColor: _MarketColors.red, inactiveColor: _MarketColors.mutedText)))] )), Expanded(flex: 5, child: Padding(padding: const EdgeInsets.all(10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(product['title'] ?? '', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _MarketColors.darkText, height: 1.2)), if (product['rating'] != null) Row(children: [const Icon(Icons.star_rounded, size: 13, color: _MarketColors.gold), const SizedBox(width: 2), Text('${product['rating']}', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: _MarketColors.darkText)), if (product['reviews_count'] != null) Text(' (${product['reviews_count']})', style: const TextStyle(fontSize: 9, color: _MarketColors.mutedText))]), Row(children: [const Icon(Icons.storefront_rounded, size: 11, color: _MarketColors.mutedText), const SizedBox(width: 3), Expanded(child: Text(_shopName(product), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9.5, color: _MarketColors.mutedText, fontWeight: FontWeight.w600)))]), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [if (isFlash && product['original_price'] != null) Text('${(product['original_price'] as num).toInt()} $symbol', style: const TextStyle(fontSize: 10, color: _MarketColors.mutedText, decoration: TextDecoration.lineThrough)), Text('$price $symbol', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 15, color: _MarketColors.red))])]))]))); }
  
  Widget _buildProductCard(Map<String, dynamic> product) { final hasDiscount = product['discount_price'] != null && product['discount_price'] < product['price']; final price = (hasDiscount ? product['discount_price'] : product['price']).toDouble(); final originalPrice = product['price'].toDouble(); final currency = product['currency'] ?? 'FC'; final symbol = currency == 'USD' ? '\$' : 'FC'; return GestureDetector(onTap: () => context.push('/market/product/${product['id']}'), child: Container(decoration: BoxDecoration(color: _MarketColors.pureWhite, borderRadius: BorderRadius.circular(18), border: Border.all(color: _MarketColors.cardBorder), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 10, offset: const Offset(0, 4))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(flex: 5, child: Stack(fit: StackFit.expand, children: [ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(18)), child: _networkImage(product['image_url'])), if (hasDiscount) Positioned(top: 8, left: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4), decoration: BoxDecoration(color: _MarketColors.red, borderRadius: BorderRadius.circular(8)), child: Text('-${((1 - price / originalPrice) * 100).round()}%', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w900)))), Positioned(top: 6, right: 6, child: Container(padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: WishlistButton(productId: product['id'].toString(), size: 20, activeColor: _MarketColors.red, inactiveColor: _MarketColors.mutedText)))])), Expanded(flex: 5, child: Padding(padding: const EdgeInsets.all(10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(product['title'] ?? '', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _MarketColors.darkText, height: 1.2)), const SizedBox(height: 3), Row(children: [const Icon(Icons.storefront_rounded, size: 11, color: _MarketColors.mutedText), const SizedBox(width: 3), Expanded(child: Text(_shopName(product), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9.5, color: _MarketColors.mutedText, fontWeight: FontWeight.w600)))]), Row(children: [const Icon(Icons.location_on_outlined, size: 11, color: _MarketColors.mutedText), const SizedBox(width: 3), Expanded(child: Text(_location(product), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9.5, color: _MarketColors.mutedText, fontWeight: FontWeight.w600)))]), const Spacer(), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.end, children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [if (hasDiscount) Text('${originalPrice.toInt()} $symbol', style: const TextStyle(decoration: TextDecoration.lineThrough, fontSize: 10, color: _MarketColors.mutedText)), Text('${price.toInt()} $symbol', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 14, color: _MarketColors.red))])), Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: _MarketColors.creamBg, borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.add_shopping_cart_rounded, size: 16, color: _MarketColors.red))])])))]))); }
  
  Widget _buildBottomNavBar() { return Container(color: _MarketColors.pureWhite, padding: const EdgeInsets.only(top: 6), child: SafeArea(top: false, child: SizedBox(height: 64, child: Stack(clipBehavior: Clip.none, alignment: Alignment.center, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_navItem(Icons.home_rounded, 'Accueil', 0), _navItem(Icons.receipt_long_rounded, 'Commandes', 1), const SizedBox(width: 60), _navItem(Icons.favorite_rounded, 'Wishlist', 3), _navItem(Icons.notifications_active_rounded, 'Alertes', 4)]), Positioned(top: -20, child: GestureDetector(onTap: _goToCart, behavior: HitTestBehavior.opaque, child: Container(width: 60, height: 60, alignment: Alignment.center, decoration: BoxDecoration(color: _MarketColors.red, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 4), boxShadow: [BoxShadow(color: _MarketColors.red.withOpacity(0.4), blurRadius: 14, offset: const Offset(0, 6))]), child: const Icon(Icons.shopping_bag_rounded, color: Colors.white, size: 26))))])))); }
  
  Widget _navItem(IconData icon, String label, int index) { final isSelected = _selectedNavIndex == index; return GestureDetector(onTap: () => _onNavTap(index), behavior: HitTestBehavior.opaque, child: Container(width: 65, padding: const EdgeInsets.symmetric(vertical: 4), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: isSelected ? _MarketColors.red : _MarketColors.mutedText, size: 24), const SizedBox(height: 2), Text(label, style: TextStyle(fontSize: 10, color: isSelected ? _MarketColors.red : _MarketColors.mutedText, fontWeight: isSelected ? FontWeight.w800 : FontWeight.w500), maxLines: 1, overflow: TextOverflow.ellipsis)]))); }
}
