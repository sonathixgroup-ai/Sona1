...
class _BuyPageState extends ConsumerState<BuyPage> with SingleTickerProviderStateMixin {
      ...
      data: (list){
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, index){
            ...
            final img = prod?['image_url'] as String??? item['image_url'] as String?;
            return Container(
              ...
              child: ListTile(
                ...
                leading: ClipRRect(
                  ...
                  child: img==null? Container(width:52,height:52,color: ivory, child: const Icon(Icons.image_rounded, size:22, color: mutedText)) : Image.network(img, width:52,height:52, fit: BoxFit.cover, errorBuilder: (_,__,___)=> Container(width:52,height:52,color: ivory, child: const Icon(Icons.broken_image_outlined))),
                ),
                title: Text(prod?['title']?? item['name']?? 'Produit', style: const TextStyle(fontWeight: FontWeight.w700, fontSize:13.5, color: darkText)),
                subtitle: Text('${(prod?['price'] as num?)?.toInt()?? item['price']?.toInt()??0} FCFA', style: const TextStyle(color: navy, fontWeight: FontWeight.w800, fontSize:12.5)),
                trailing: InkWell(
                  borderRadius: BorderRadius.circular(20),
  ...
  Widget _buildEmptyStateWithAction(String title, String subtitle, IconData icon, VoidCallback onAction){
    return Center(child: Padding(padding: const EdgeInsets.symmetric(horizontal:32), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(width:90,height:90, decoration: BoxDecoration(color: navyDeep.withValues(alpha:0.06), shape: BoxShape.circle, border: Border.all(color: gold.withValues(alpha:0.4), width:1.4)), child: const Icon(Icons.favorite_border_rounded, size:38, color: navy)),
  ...
  void _showAdvancedFilters(){ showModalBottomSheet(context: context, isScrollControlled:true, backgroundColor: Colors.transparent, builder: (_)=> const AdvancedFiltersSheet()); }
  void _createWishlist(){ showDialog(context: context, builder: (_)=> const CreateWishlistDialog()); }
}
