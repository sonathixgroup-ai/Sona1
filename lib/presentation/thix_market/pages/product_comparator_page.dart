...
final comparatorIdsProvider = StateNotifierProvider<ComparatorNotifier, List<String>>((ref) {
...
class ProductComparatorPage extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (ids.isNotEmpty)
            TextButton(
              onPressed: () => ref.read(comparatorIdsProvider.notifier).clear(), 
              child: const Text('Vider', style: TextStyle(color: _MarketColors.red))
            ),
        ],
      ...
      body: productsAsync.when(
        ...
        data: (products) {
          if (products.length < 2) {
            return _empty(context, products);
          }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              hasOne ? 'Ajoutez un autre produit' : 'Comparateur vide',
              style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: _MarketColors.darkText)
            ),
            const SizedBox(height: 8),
            Text(
              hasOne ? 'Il vous faut au moins 2 produits.' : 'Sélectionnez des produits et cliquez sur comparer.',
  ...
  Widget _header(BuildContext context, WidgetRef ref, Map<String, dynamic> product) {
    ...
    if (product['image_url'] != null) {
      img = product['image_url'].toString();
    }
        ...
        children: [
          Stack(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () => ref.read(comparatorIdsProvider.notifier).remove(id),
                child: Container(
        ...
        children: [
          SizedBox(
            ...
            child: Padding(
              ...
              child: Text(
                feature['label']!,
                style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: _MarketColors.mutedText)
              )
            )
          ),
          ...products.map((p) {
