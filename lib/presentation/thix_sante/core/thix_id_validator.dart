...
// Role: Validateur et generateur officiel THIX ID
...
class ThixIdValidator {
    ...
    final String complementPart = String.fromCharCodes(
      Iterable.generate(
        ...
        (_) => _alphabet.codeUnitAt(
          secureRand.nextInt(_alphabet.length),
        ),
      ),
    );
  ...
  static bool isValidFormat(String thixId) {
    ...
    final RegExp pattern =
        RegExp(r'^THIX-[A-Z]{2}-\d{4}-\d{5}-[A-Z]{3}-\d$');

    return pattern.hasMatch(thixId.trim().toUpperCase());
  }

  static String clean(String thixId) =>
      thixId.trim().toUpperCase();

  static List<String> get availableCountries =>
      _countryCodes.keys.toList();

  static String getCountryCode(String country) =>
      _countryCodes[country] ?? 'XX';
}
