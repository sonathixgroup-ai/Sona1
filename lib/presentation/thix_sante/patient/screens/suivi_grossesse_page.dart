...
class _SuiviGrossessePageState extends ConsumerState<SuiviGrossessePage> with SingleTickerProviderStateMixin {
  ...
  Timer? _cTimer; int _cSec=0; DateTime? _lastC;
  ...
  Widget _kickCounter(){
    ...
    return kicksAsync.when(data: (list){
  ...
  Widget _tabMaman(){
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      Container(height:160, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)), child: vitals.when(data: (list){
      ...
      const SizedBox(height:12),
      vitals.when(data: (list)=> Column(children: list.take(10).map((v)=> ListTile(dense:true, title: Text('${v.type}: ${v.value}'), subtitle: Text(v.createdAt.toString().substring(0,16)),)).toList()), loading: ()=> const CircularProgressIndicator(), error: (_,__ )=> const Text('Erreur')),
    ]);
  }
  ...
  Widget _tabDocs(){
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      const SizedBox(height:12),
      records.when(data: (list)=> Column(children: list.map((r)=> Card(child: ListTile(title: Text(r.title, style: const TextStyle(fontSize:12, fontWeight: FontWeight.w700)), subtitle: Text(r.description??'', style: const TextStyle(fontSize:10)), trailing: IconButton(icon: const Icon(Icons.visibility), onPressed: (){})))).toList()), loading: ()=> const CircularProgressIndicator(), error: (e,_ )=> Text('Erreur $e')),
    ]);
  }
  ...
  Widget _tabPrepa(PregnancyProfile p){
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      const Text('⏱️ Contractions', style: TextStyle(fontWeight: FontWeight.w900)), const SizedBox(height:8),
      Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)), child: Column(children: [
        Text('$_cSec s', style: const TextStyle(fontSize:28, fontWeight: FontWeight.w900)), const SizedBox(height:8),
        ...
        const Divider(),
        contractions.when(data: (list){ if(list.isEmpty) return const Text('Aucune', style: TextStyle(fontSize:11)); final avgDur = list.map((e)=> e.durationSec).reduce((a,b)=> a+b)/list.length; final avgInter = list.length>1? list.map((e)=> e.intervalSec).reduce((a,b)=> a+b)/list.length : 0; return Column(children: [Text('Moyenne: ${avgDur.toStringAsFixed(0)}s durée / ${(avgInter/60).toStringAsFixed(0)} min intervalle', style: const TextStyle(fontSize:11, fontWeight: FontWeight.bold)),...list.take(5).map((c)=> ListTile(dense:true, title: Text('${c.durationSec}s'), subtitle: Text('Intervalle ${(c.intervalSec/60).toStringAsFixed(0)} min')))]); }, loading: ()=> const CircularProgressIndicator(), error: (_,__ )=> const Text('Erreur')),
      ])),
      const SizedBox(height:12),
  ...
  Widget _createProfile(){
    final ctrl = TextEditingController(); PregnancyType selectedType = PregnancyType.singleton;
    return StatefulBuilder(builder: (context, setLocal){ return Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Text('🤰', style: TextStyle(fontSize:50)), const Text('Créer suivi', style: TextStyle(fontWeight: FontWeight.w800, fontSize:18)),
      ...
      const SizedBox(height:12), SizedBox(width: double.infinity, height:48, child: ElevatedButton(onPressed: () async { if(ctrl.text.isEmpty) return; await ref.read(grossesseServiceProvider).createProfile(pid, DateTime.parse(ctrl.text), selectedType); ref.invalidate(grossesseProfileProvider(pid)); }, child: const Text('COMMENCER'))),
    ]))); });
  }
  ...
  void _addJournalPhoto() async {
    final picker = ImagePicker(); final file = await picker.pickImage(source: ImageSource.gallery);
    if(file==null) return; final bytes = await file.readAsBytes();
    ...
    if(!mounted) return; showDialog(context: context, builder: (_)=> AlertDialog(title: const Text('Journal'), content: TextField(controller: t), actions: [TextButton(onPressed: () async { await ref.read(grossesseServiceProvider).addJournal(pid, t.text, 'Photo ventre', photoUrl: url); ref.invalidate(journalProvider(pid)); if(mounted) Navigator.pop(context); }, child: const Text('Enregistrer'))]));
  }
  void _pickDoc() async { final res = await FilePicker.platform.pickFiles(type: FileType.custom, allowedExtensions: ['pdf','jpg','png']); if(res==null) return; final bytes = res.files.first.bytes; if(bytes==null) return; await ref.read(grossesseServiceProvider).uploadDoc(pid, res.files.first.name, bytes); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Document uploadé'))); }
  ...
  Future<void> _exportPdf() async {
    final profile = ref.read(grossesseProfileProvider(pid)).value; if(profile==null) return;
    final vitals = ref.read(vitalsProvider(pid)).value?? [];
    final doc = pw.Document(); doc.addPage(pw.Page(build: (c)=> pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [pw.Text('Suivi Grossesse - DPA ${profile.dpa.day}/${profile.dpa.month}/${profile.dpa.year}'), pw.SizedBox(height:10),...vitals.map((v)=> pw.Text('${v.type}: ${v.value} - ${v.createdAt.toString().substring(0,10)}'))])));
