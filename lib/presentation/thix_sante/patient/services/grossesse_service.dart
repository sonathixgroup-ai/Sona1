...
Future<void> uploadDoc(String? patientId, String fileName, dynamic bytes) async {
  ...
  Future<void> addConsultation(String? pid, String title, String desc) async {
    await _db.from('health_records').insert({
      'patient_uid': resolveUid(pid), 'professional_uid': _uid,
  ...
  List<String> calculateRisks({required int sa, List<PregnancyVital> vitals = const [], List<PregnancyKick> kicks = const [], List<PregnancyContraction> contractions = const []}){
    ...
    if(ta.isNotEmpty){ final sys = int.tryParse(ta.first.value.split('/').first)??0; if(sys>=140) alerts.add('🚨 TA ${ta.first.value} >140 - Risque pré-éclampsie'); }
    if(kicks.where((k)=> k.createdAt.day==DateTime.now().day).isEmpty && sa>=28) alerts.add('⚠️ 0 mouvement aujourd\'hui après 28 SA');
    if(sa>=37 && contractions.length>=3){ final avg = contractions.take(3).map((c)=> c.intervalSec).reduce((a,b)=> a+b)/3; if(avg<=300) alerts.add('🚨 Contractions toutes les 5 min - Maternité'); }
    return alerts;
  }
}
