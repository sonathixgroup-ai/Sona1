...
class AppointmentService {
  ...
  Future<List<Map<String,dynamic>>> getMyLinkedDoctors() async {
    final res = await _db
     .from('health_links')
     .select('doctor_id, doctors!inner(id, full_name, specialite, avatar_url, adresse)')
     .eq('patient_id', _uid)
     .eq('is_active', true);
  ...
  Future<List<String>> getTakenSlots({required String doctorId, required DateTime date}) async {
    ...
    final res = await _db.from('appointments')
     .select('creneau')
  ...
  Future<AppointmentModel> createAppointment({
    required String doctorId, required DateTime date, required String creneau,
    required String type, required String motif,
  }) async {
