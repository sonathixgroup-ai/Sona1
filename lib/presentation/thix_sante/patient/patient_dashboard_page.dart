...
// Spec: 20 Services rapides [18 - Don d'organes + 3 nouveaux], Stats temps reel
// =============================================================================
...
class PatientDashboardPage extends ConsumerWidget {
  ...
  Widget _buildAppBar() {
    return SliverAppBar(
      ...
      toolbarHeight: 64,
      leading: IconButton(
        icon: const Icon(Icons.menu_rounded, color: ThixSanteColors.ink),
        onPressed: () {},
      ),
      title: Column(
        ...
        children: [
          ...
          const Text(
            ...
            style: TextStyle(fontSize: 11, color: ThixSanteColors.muted),
          ),
        ],
      ),
      actions: [
        Stack(
          children: [
            IconButton(
              icon: const Icon(Icons.notifications_none_rounded, color: ThixSanteColors.ink),
              onPressed: () {},
            ),
            Positioned(
              top: 10,
              ...
              child: Container(
                width: 18,
                height: 18,
                decoration: const BoxDecoration(color: ThixSanteColors.danger, shape: BoxShape.circle),
                child: const Center(
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            ...
            child: Column(
              ...
              children: [
                const Row(
                  children: [
                    Text('Bonjour, Alex', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                    SizedBox(width: 4),
                ...
                Row(
                  children: [
                    ElevatedButton.icon(
                      ...
                      label: const Text('Dossier de sante', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 12)),
                    ),
                      ...
                      child: const Row(
                        children: [
                          ...
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('Score de sante', style: TextStyle(color: Colors.white70, fontSize: 9)),
                              Text('85%', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 16)),
                            ],
                          ),
                        ],
                      ),
          ...
          Expanded(
            flex: 2,
            child: Image.network('https://cdn3d.iconscout.com/3d/premium/thumb/doctor-3d-icon-download-in-png-blend-fbx-gltf-file-formats--medical-health-care-pack-medical-icons-5183886.png', height: 140, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Icon(Icons.medical_services_rounded, color: Colors.white, size: 80)),
          ),
        ],
      ),
    );
  ...
  Widget _statCard({required String icon, required String value, required String label1, required String label2, required Color color}) {
    return Container(
      ...
      child: Row(
        children: [
          Container(width: 36, height: 36, decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(10)), child: Center(child: Text(icon, style: const TextStyle(fontSize: 18)))),
          const SizedBox(width: 8),
  ...
  Widget _buildServicesRapides(BuildContext context) {
    final List<Map<String, dynamic>> items = [
      {'label': 'Consulter\nmedecin', 'icon': '🩺', 'color': const Color(0xFFDBEAFE)},
      {'label': 'Dossier\nmedical', 'icon': '📁', 'color': const Color(0xFFDBEAFE)},
      {'label': 'Resultats\nexamens', 'icon': '🧪', 'color': const Color(0xFFD1FAE5)},
      {'label': 'Mes\nordonnances', 'icon': '📋', 'color': const Color(0xFFEDE9FE)},
      ...
      children: [
        ...
        Padding(
          ...
          child: GridView.builder(
            ...
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 6, childAspectRatio: 0.78, crossAxisSpacing: 8, mainAxisSpacing: 8),
            itemCount: items.length,
            itemBuilder: (c, i) {
              final it = items[i];
              return InkWell(
                onTap: () { if (it['page']!= null) Navigator.push(c, MaterialPageRoute(builder: (_) => it['page'] as Widget)); },
  ...
  Widget _buildServicesSante(BuildContext context) {
    final List<Map<String, String>> sante = [
