  ...
  Future<void> _save() async {
    ...
    try{
      ...
      if(_coverFile!=null){
        ...
        // Simulation de progression (Supabase SDK n'a pas de callback natif, on simule fluide)
        for(var i=0; i<=10; i++){ await Future.delayed(Duration(milliseconds: 80)); if(mounted) setState(()=> _coverProgress=i/10); }
      ...
      if(_videoFile!=null){
        ...
        for(var i=0; i<=10; i++){ await Future.delayed(Duration(milliseconds: 150)); if(mounted) setState(()=> _videoProgress=i/10); }
    ...
    return Container(
      ...
      child: Form(
        ...
        child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(child: InkWell(onTap: _saving? null : _pickCover, child: Container(height: 110, decoration: BoxDecoration(color: Color(0xFFF7FAFF), borderRadius: BorderRadius.circular(14), border: Border.all(color: Color(0xFFE7EEFC))), child: _coverFile!=null? Center(child: Icon(Icons.check_circle_rounded, color: Colors.green)) : widget.existing!=null? ClipRRect(borderRadius: BorderRadius.circular(14), child: Image.network(widget.existing!.coverUrl, fit: BoxFit.cover, width: double.infinity)) : Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.image_rounded, color: Color(0xFF2D6CDF)), Text('Cover *')]))))),
            SizedBox(width: 12),
            Expanded(child: InkWell(onTap: _saving? null : _pickVideo, child: Container(height: 110, decoration: BoxDecoration(color: Color(0xFFF7FAFF), borderRadius: BorderRadius.circular(14), border: Border.all(color: Color(0xFFE7EEFC))), child: _videoFile!=null? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.check_circle_rounded, color: Colors.green), Text('${(_videoFile!.size/1024/1024).toStringAsFixed(1)} Mo', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700))])) : Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.video_library_rounded, color: Color(0xFF2D6CDF)), Text('Vidéo MP4 *')]))))),
          ]),
          SizedBox(height: 16),
          ...
          SizedBox(height: 12),
          Row(children: [
            Expanded(child: DropdownButtonFormField(value: _type, items: ['Films','Séries','Vidéos','Musique'].map((e)=>DropdownMenuItem(value:e, child: Text(e))).toList(), onChanged: (v)=>setState(()=>_type=v!), decoration: InputDecoration(labelText: 'Type', filled: true, fillColor: Color(0xFFF7FAFF), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))),
            SizedBox(width: 12),
            Expanded(child: TextFormField(controller: _year, decoration: InputDecoration(labelText: 'Année', filled: true, fillColor: Color(0xFFF7FAFF), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))),
          ]),
          ...
          SizedBox(height: 18),
          SizedBox(width: double.infinity, height: 52, child: ElevatedButton(onPressed: _saving? null : _save, style: ElevatedButton.styleFrom(backgroundColor: Color(0xFF0A1F44), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))), child: _saving? SizedBox(width:18,height:18, child: CircularProgressIndicator(strokeWidth:2, color: Colors.white)) : Text(widget.existing==null?'Uploader et Publier':'Enregistrer', style: TextStyle(fontWeight: FontWeight.w800)))),
        ])),
      ),
    );
  }
