...
class _ThixMediaPageState extends ConsumerState<ThixMediaPage> {
 ...
 Future<void> _loadMoreFil() async {
          ...
          child: PageView.builder(
            ...
            itemBuilder:(c,idx){
              ...
              return Stack(fit:StackFit.expand, children:[
                ...
                Positioned(
                  left:20, bottom:textBottom, right:20,
                  child:IgnorePointer(
                    child:Column(
  ...
  Widget _header(){ 
    ...
    return ClipRRect(
      child:BackdropFilter(
        ...
        child:Container(
          height:60, padding:const EdgeInsets.only(top:8,left:12,right:12,bottom:8), 
          ...
          child:Row(
            children:[ 
                ...
                child:Container(
                  ...
                  child:Row(
                    children:[ 
                      ...
                      Expanded(child:TextField(controller:_searchController, focusNode:_searchFocusNode, onChanged:_onSearchChanged, style:const TextStyle(color:Colors.white,fontSize:13), decoration:const InputDecoration(hintText:"Rechercher...", hintStyle:TextStyle(color:Colors.white30,fontSize:13), border:InputBorder.none, isDense:true, contentPadding:EdgeInsets.zero)))
                    ]
                  )
                )
              ), 
              ...
              if(isAdmin)...[
                GestureDetector(onTap:()=>Navigator.push(context, MaterialPageRoute(builder:(_)=>const ThixMediaAdminPage())), child:Container(width:32,height:32, decoration:BoxDecoration(shape:BoxShape.circle,color:kRed.withOpacity(0.15), border:Border.all(color:kRed.withOpacity(0.3))), child:const Icon(Icons.admin_panel_settings_rounded,color:kRed,size:16))), 
  ...
  // BOTTOM NAV COMPLET RESTAURÉ AVEC LA LOGIQUE DE DONNÉES
          ...
          child:Container(
            ...
            child:Row(
              ...
              children:[
                GestureDetector(
                  onTap:(){ 
                    if(creatorId.isNotEmpty) Navigator.push(context, MaterialPageRoute(builder:(_)=>UserProfilePage(userId:creatorId))); 
                  ...
                  child:Column(
                    ...
                    children:[
                      ...
                      SizedBox(width:55, child:Text(displayName, maxLines:1, overflow:TextOverflow.ellipsis, textAlign:TextAlign.center, style:const TextStyle(fontSize:10,fontWeight:FontWeight.w500,color:Colors.white70)))
                    ]
                  )
                ),
                ...
                GestureDetector(
                  ...
                  child:Column(mainAxisSize:MainAxisSize.min, mainAxisAlignment:MainAxisAlignment.center, children:const [Icon(Icons.add_circle_outline_rounded,color:kRed,size:22), SizedBox(height:4), Text('Poster', style:TextStyle(fontSize:10,fontWeight:FontWeight.bold,color:kRed))])
                ),
              ],
            ),
          ),
...
// VIDEO PLAYER COMPLET RESTAURÉ (Avec barre de progression drag & drop)
...
class _FeedVideoPlayerState extends State<FeedVideoPlayer> {
  ...
  @override Widget build(BuildContext context){
    ...
    return GestureDetector(
      onTap:(){ if(_c.value.isPlaying){ _c.pause(); _paused=true; } else { _c.play(); _paused=false; } setState((){}); widget.onPlayStateChanged(_paused); }, 
      child:Stack(
        fit:StackFit.expand, 
        children:[
          ...
          AnimatedPositioned(
            duration: const Duration(milliseconds: 250), curve: Curves.easeInOutCubic, left: 0, right: 0, bottom: 80, 
            child: GestureDetector(
              ...
              child: Container(
                height: 24, color: Colors.transparent, alignment: Alignment.bottomCenter,
...
// COMMENTS SHEET COMPLET RESTAURÉ (Avec replies, likes et design bottom sheet)
...
class _CommentsSheetState extends ConsumerState<_CommentsSheet> {
  ...
  Widget _tile(CommentItem c,{bool reply=false}){ final liked=_liked.contains(c.id); return Padding(padding:EdgeInsets.only(left:reply?40:0, top:12), child:Row(crossAxisAlignment:CrossAxisAlignment.start, children:[ GestureDetector(onTap: ()=>Navigator.push(context, MaterialPageRoute(builder:(_)=>UserProfilePage(userId:c.userId))), child:CircleAvatar(radius:reply?12:16, backgroundColor:kSurfaceLight, backgroundImage:c.avatarUrl!=null && c.avatarUrl!.isNotEmpty? NetworkImage(c.avatarUrl!):null, child:c.avatarUrl==null? Icon(Icons.person,size:reply?14:18,color:kTextGrey):null)), const SizedBox(width:10), Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start, children:[ Row(children:[GestureDetector(onTap: ()=>Navigator.push(context, MaterialPageRoute(builder:(_)=>UserProfilePage(userId:c.userId))), child:Text(c.userName, style:const TextStyle(color:Colors.white,fontSize:13,fontWeight:FontWeight.w700))), const SizedBox(width:8), Text(c.createdAt.toString().substring(0,16), style:const TextStyle(color:kTextGrey,fontSize:11))]), Text(c.content, style:const TextStyle(color:Colors.white70,fontSize:13)), const SizedBox(height:6), Row(children:[ GestureDetector(onTap:(){ setState(()=>_replyingTo=c); _focus.requestFocus(); }, child:const Text('Répondre', style:TextStyle(color:kTextGrey,fontSize:12,fontWeight:FontWeight.bold))), const SizedBox(width:20), GestureDetector(onTap:(){ setState(()=>liked?_liked.remove(c.id):_liked.add(c.id)); Supabase.instance.client.rpc('toggle_comment_like', params:{'p_comment_id': c.id}); }, child:Row(children:[Icon(liked?Icons.favorite:Icons.favorite_border, color:liked?kRed:kTextGrey, size:14), const SizedBox(width:4), Text(c.likeCount>0? '${c.likeCount}':"J'aime", style:TextStyle(color:liked?kRed:kTextGrey,fontSize:11))]))]), if(!reply && (c.replyCount>0||_replies.containsKey(c.id)))...[const SizedBox(height:6), GestureDetector(onTap:(){ if(_expanded.contains(c.id)) setState(()=>_expanded.remove(c.id)); else _fetchReplies(c.id); }, child:Row(children:[Container(width:24,height:1,color:kBorderLight), const SizedBox(width:8), Text(_expanded.contains(c.id)?'Masquer':'Voir ${c.replyCount} réponses', style:const TextStyle(color:kTdiaBlue,fontSize:12,fontWeight:FontWeight.w600))]))], if(!reply && _expanded.contains(c.id))...[...(_replies[c.id]??[]).map((r)=>_tile(r, reply:true)) ]])) ])); }
