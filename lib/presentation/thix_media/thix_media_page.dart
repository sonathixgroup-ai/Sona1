...
class _ThixMediaPageState extends ConsumerState<ThixMediaPage> {
  ...
  Widget _skeleton() {
    return ListView(
      ...
      children: [
        Row(
          children: [
            Container(height: 36, width: 36, decoration: BoxDecoration(color: kSurface, borderRadius: BorderRadius.circular(10))),
            const SizedBox(width: 12),
            Container(height: 20, width: 120, decoration: BoxDecoration(color: kSurface, borderRadius: BorderRadius.circular(6))),
          ],
        ),
        const SizedBox(height: 24),
        Container(height: 520, decoration: BoxDecoration(color: kSurface, borderRadius: BorderRadius.circular(24))),
        const SizedBox(height: 24),
        Container(height: 90, decoration: BoxDecoration(color: kSurface, borderRadius: BorderRadius.circular(16))),
      ],
    );
  }
  ...
  Widget _header() {
    return ClipRRect(
      child: BackdropFilter(
        ...
        child: Container(
          decoration: BoxDecoration(color: kBg.withOpacity(0.75), border: Border(bottom: BorderSide(color: Colors.white.withOpacity(0.06)))),
          ...
          child: Row(
            children: [
              Container(
                width: 36,
                height: 36,
                decoration: BoxDecoration(gradient: const LinearGradient(colors: [kViolet, kVioletDark]), borderRadius: BorderRadius.circular(10)),
                child: const Icon(Icons.play_arrow_rounded, size: 22, color: Colors.white),
              ),
              const SizedBox(width: 10),
              const Text.rich(TextSpan(children: [
                TextSpan(text: 'THIX ', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 19, color: Colors.white)),
                TextSpan(text: 'MEDIA', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 19, color: kViolet)),
              ])),
              const Spacer(),
              ...
              Container(
                width: 36,
                height: 36,
                decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.white.withOpacity(0.12), width: 1.5)),
                child: Container(margin: const EdgeInsets.all(2), decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [kGold, kGold2])), child: const Icon(Icons.person_rounded, size: 18, color: Colors.black)),
              ),
            ],
          ),
        ),
  ...
  Widget _iconBtn(IconData icon, VoidCallback onTap, {bool hasBg = false, bool isActive = false}) {
    return InkWell(
      onTap: onTap,
      ...
      child: Container(
        width: 38,
        height: 38,
        decoration: BoxDecoration(color: isActive? kViolet : hasBg? kSurface : kSurface.withOpacity(0.8), borderRadius: BorderRadius.circular(12), border: Border.all(color: isActive? kViolet : Colors.white.withOpacity(0.08))),
        child: Icon(icon, size: 20, color: isActive? Colors.white : kTextWhite),
      ),
    );
  }
  ...
  Widget _searchBar() {
    return Container(
      ...
      child: Container(
        height: 50,
        padding: const EdgeInsets.symmetric(horizontal: 16),
        decoration: BoxDecoration(color: kSurface, borderRadius: BorderRadius.circular(14), border: Border.all(color: kViolet.withOpacity(0.3))),
        child: Row(
          children: [
            ...
            Expanded(
              child: TextField(
                ...
                style: const TextStyle(fontSize: 14.5, color: kTextWhite),
                decoration: const InputDecoration(hintText: 'Films, séries...', border: InputBorder.none, hintStyle: TextStyle(fontSize: 14, color: kTextGrey), isDense: true),
              ),
            ),
          ],
        ),
  ...
  Widget _bannerCinema(List<MediaContent> bannerItems) {
    return Column(
      children: [
        SizedBox(
          height: 540,
            ...
            itemBuilder: (context, idx) {
              ...
              return AnimatedScale(
                ...
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 500),
                  margin: EdgeInsets.symmetric(horizontal: 6, vertical: isCenter? 0 : 16),
                          ...
                          Container(
                            decoration: BoxDecoration(
                              gradient: LinearGradient(
                                ...
                                colors: [Colors.black.withOpacity(0.2), Colors.transparent, Colors.black.withOpacity(0.95)],
                              ),
                            ),
                          ),
                          Positioned(
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(item.title.toUpperCase(), maxLines: 2, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: Colors.white)),
                                  const SizedBox(height: 10),
                                  Text('${item.type} • ${item.year?? 2024}', style: const TextStyle(fontSize: 12, color: Colors.white70)),
                                  const SizedBox(height: 18),
                                  ElevatedButton.icon(
                                    onPressed: () => _navigateToVideo(item),
                                    icon: const Icon(Icons.play_arrow_rounded, color: Colors.black),
                                    label: const Text('Lecture', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w800)),
                                    style: ElevatedButton.styleFrom(backgroundColor: Colors.white, minimumSize: const Size(0, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
                                  ),
                                ],
                              ),
                            ),
        ...
        Row(
          ...
          children: List.generate(bannerItems.length, (i) {
            ...
            return AnimatedContainer(
              ...
              width: active? 28 : 6,
              height: 6,
              decoration: BoxDecoration(color: active? kViolet : Colors.white.withOpacity(0.18), borderRadius: BorderRadius.circular(10)),
            );
          }),
        ),
      ],
  ...
  Widget _sectionTitle(String title, {IconData? icon, Color iconColor = kViolet}) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Row(
          children: [
            if (icon!= null) Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: iconColor.withOpacity(0.15), borderRadius: BorderRadius.circular(8)), child: Icon(icon, size: 16, color: iconColor)),
            if (icon!= null) const SizedBox(width: 10),
            Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: kTextWhite)),
          ],
        ),
      ],
    );
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              final img = allMedia.where((e) => e.type == cat['type']).isNotEmpty? allMedia.where((e) => e.type == cat['type']).first.coverUrl : '';
              return GestureDetector(
                onTap: () => _goToCategoryAndScroll(cat['type'] as String),
                child: Container(
                  width: 150,
                  decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: selected? (cat['color'] as Color) : Colors.white.withOpacity(0.08), width: selected? 1.5 : 1)),
                  child: ClipRRect(
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [(cat['color'] as Color).withOpacity(0.65), Colors.black.withOpacity(0.85)]))),
                        Center(child: Text(cat['label'] as String, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800))),
                      ],
                    ),
                  ),
                ),
  ...
  Widget _tendances() {
    ...
    if (trending.isEmpty) return const Text('Aucune tendance', style: TextStyle(color: kTextGrey));
    return SizedBox(
      height: 220,
      child: ListView.separated(
        scrollDirection: Axis.horizontal,
        ...
        itemBuilder: (context, i) {
          ...
          return GestureDetector(onTap: () => _navigateToVideo(item), child: ClipRRect(borderRadius: BorderRadius.circular(16), child: CachedNetworkImage(imageUrl: item.coverUrl, height: 200, width: 124, fit: BoxFit.cover)));
        },
      ),
    );
  }
          ...
          return GestureDetector(
            ...
            child: SizedBox(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ClipRRect(borderRadius: BorderRadius.circular(14), child: CachedNetworkImage(imageUrl: item.coverUrl, height: 190, width: 138, fit: BoxFit.cover)),
                  const SizedBox(height: 10),
  ...
  Widget _premiumBanner() {
    return Container(
      ...
      decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF1E1A12), Color(0xFF17101E)]), borderRadius: BorderRadius.circular(21)),
    ...
    return Padding(
      ...
      child: ClipRRect(
        ...
        child: BackdropFilter(
          ...
          child: Container(
            height: 70,
            decoration: BoxDecoration(color: const Color(0xFF12121A).withOpacity(0.88), borderRadius: BorderRadius.circular(28)),
            child: Row(
  ...
  Widget _navItem(IconData icon, String label, bool selected, int idx) {
    return InkWell(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: selected? kViolet : kTextGrey, size: 22), const SizedBox(height: 3), Text(label, style: TextStyle(fontSize: 9.5, color: selected? kViolet : kTextGrey))]),
    );
  }
}
