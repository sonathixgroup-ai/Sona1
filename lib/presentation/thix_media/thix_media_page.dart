...
/// Flux live du compteur (likes/vues/commentaires)
/// C'est l'approche scalable : on écoute uniquement la vidéo visible, pas tout le feed.
...
class _ThixMediaPageState extends ConsumerState<ThixMediaPage> {
  ...
  void _registerView(MediaContent item) {
    ...
    _mediaService.registerView(item.id); // L'API s'occupe de l'incrémentation scalable
  }
  ...
  Future<void> _toggleLike(MediaContent item) async {
    ...
    setState(() {
      if (wasLiked) _likedMediaIds.remove(item.id);
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erreur serveur : $e'), backgroundColor: kRed));
      setState(() {
        if (wasLiked) _likedMediaIds.add(item.id);
        else _likedMediaIds.remove(item.id);
      });
    }
  }
          ...
          itemBuilder: (context, index) {
            ...
            return GestureDetector(
              ...
              child: Stack(fit: StackFit.expand, children: [
                // Vidéo en mode Fill complet avec contrôles de progression VLC
                FeedVideoPlayer(videoUrl: item.videoUrl, coverUrl: item.coverUrl, isPlaying: isFocused),

                // Informations texte nues
                Positioned(
                  ...
                  child: IgnorePointer(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(item.title, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900, height: 1.1, shadows: [Shadow(color: Colors.black54, blurRadius: 4)])),
                        if (item.subtitle != null) ...[
                          const SizedBox(height: 6),
                          Text(item.subtitle!, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 13, height: 1.4, shadows: [Shadow(color: Colors.black54, blurRadius: 4)])),
                        ],
                      ]
                    ),
                  ),
  ...
  Widget _bottomNav(String selectedCategory, MediaContent? currentItem) {
    ...
    // Intégration Scalable : On récupère les compteurs live de la base de données
    ...
    final List<_NavItemData> items = isFil
        ? [
            ...
            _NavItemData(
              // COMPTAGE REEL (Affiché dynamiquement via Supabase Realtime)
...
// ---------------- LECTEUR VIDÉO DU FIL (Plein écran pur + Barre VLC fine) ----------------
...
class _FeedVideoPlayerState extends State<FeedVideoPlayer> {
  ...
  // Variables pour la barre VLC/TikTok
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Stack(fit: StackFit.expand, children: [
        ...
        // ---------------- BARRE DE PROGRESSION FINE (Style TikTok/VLC) ----------------
        Positioned(
          left: 0,
          right: 0,
          bottom: 84, // Positionnée juste au dessus de la bottomNav
          child: GestureDetector(
            ...
            child: ValueListenableBuilder<Duration>(
              ...
              builder: (context, pos, child) {
                ...
                return Container(
                  height: 24, // Zone tactile assez grande pour le doigt
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      // Affichage du chronomètre pendant le glissement (comme VLC)
                      if (_isDragging)
                        Positioned(
                          top: -30,
                          left: (MediaQuery.of(context).size.width * percent) - 25,
                          child: Text(
                            _fmt(pos),
...
// ---------------- SHEET DE COMMENTAIRES (Scalable, Imbriqués, Éditables) ----------------
// Pour la scalabilité : on ne charge QUE les racines. Les réponses (sous-commentaires)
// se chargent uniquement au clic. C'est l'architecture parfaite pour des millions de vues.
...
class _CommentsSheetState extends ConsumerState<_CommentsSheet> {
  ...
  Future<void> _loadFirstPage() async {
    ...
    if (!mounted) return;
    setState(() {
      _rootComments..clear()..addAll(page.items);
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      // Les réponses sans perte d'espace
