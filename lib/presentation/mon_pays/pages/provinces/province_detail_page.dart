          ...
          slivers: [
            ...
            SliverPadding(
              ...
              sliver: SliverList.list(children: [
                ...
                const SizedBox(height: 12),
                _ExploreGrid(province: province),
...
// ==================== EXPLORE: 2 PAR LIGNE CARRES AVEC TEXTE + PHOTO ====================
class _ExploreGrid extends StatelessWidget {
  final Province province;
  const _ExploreGrid({required this.province});
  @override
  Widget build(BuildContext context) {
    final items = [
      _ExploreItem(icon: Icons.public_rounded, title: 'Culture', subtitle: province.languages?? 'Langues & Traditions', color: const Color(0xFF123B7A)),
      _ExploreItem(icon: Icons.monetization_on_rounded, title: 'Économie', subtitle: '${province.economicResources.length} secteurs', color: const Color(0xFF2E7D32)),
      _ExploreItem(icon: Icons.landscape_rounded, title: 'Tourisme', subtitle: '${province.tourismSites.length} sites', color: const Color(0xFF1565C0)),
      _ExploreItem(icon: Icons.emergency_rounded, title: 'Urgences', subtitle: '${province.emergencyContacts.length} numéros', color: const Color(0xFFD32F2F)),
      _ExploreItem(icon: Icons.dashboard_customize_rounded, title: 'Admin', subtitle: '${province.administrativeDivisions.length} divisions', color: const Color(0xFF6A1B9A)),
      _ExploreItem(icon: Icons.emoji_events_rounded, title: 'Réalisations', subtitle: 'Projets majeurs', color: const Color(0xFFE3B23C)),
      _ExploreItem(icon: Icons.perm_media_rounded, title: 'Galerie', subtitle: 'Photos & Vidéos', color: const Color(0xFF00695C)),
    ];
    return GridView.builder(
      shrinkWrap: true, physics: const NeverScrollableScrollPhysics(),
      ...
      itemCount: items.length,
      itemBuilder: (_, i) => _SquareExploreCard(item: items[i], province: province),
    );
  }
}
class _ExploreItem { final IconData icon; final String title, subtitle; final Color color; _ExploreItem({required this.icon, required this.title, required this.subtitle, required this.color}); }

class _SquareExploreCard extends StatelessWidget {
  final _ExploreItem item; final Province province;
  const _SquareExploreCard({required this.item, required this.province});
  @override
  Widget build(BuildContext context) {
    return Material(
      color: Colors.white,
      ...
      child: InkWell(
        ...
        child: Container(
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFFE7EAF3))),
          padding: const EdgeInsets.all(14),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(width: 44, height: 44, decoration: BoxDecoration(color: item.color.withOpacity(0.12), borderRadius: BorderRadius.circular(12)), child: Icon(item.icon, color: item.color, size: 24)),
            const Spacer(),
            Text(item.title, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 14, color: Color(0xFF0A1F44))),
            const SizedBox(height: 2),
            Text(item.subtitle, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, color: Color(0xFF6B7690), height: 1.3)),
            const SizedBox(height: 8),
            Row(children: [Text('Voir', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: item.color)), const SizedBox(width: 4), Icon(Icons.arrow_forward_rounded, size: 14, color: item.color)]),
          ]),
        ),
      ),
    );
  ...
  void _handleTap(BuildContext context) {
    // TODO: ouvrir modals existantes _showCultureModal etc.
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${item.title} - à implémenter avec ta modal existante')));
  }
}
