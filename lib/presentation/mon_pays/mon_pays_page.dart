...
// FIX SYNTAX - Build OK - Graphite + Président en grand
...
class _MonPaysPageState extends ConsumerState<MonPaysPage> {
  ...
  final List<Map<String, String>> patrioticPosters = [
    {'title': 'Unite Nationale', 'subtitle': 'Bendele ya Congo', 'img': 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4'},
    {'title': 'Devoir Civique', 'subtitle': 'Engager pour la Patrie', 'img': 'https://images.unsplash.com/photo-1529156069898-49953e39b3ac'},
    {'title': 'Memoire Collective', 'subtitle': 'Honorer nos Heros', 'img': 'https://images.unsplash.com/photo-1497895121-66bdc4d7d3b2'},
    {'title': 'Travail et Progres', 'subtitle': 'Batir la RDC', 'img': 'https://images.unsplash.com/photo-1516026672322-bc52d61a55e5'},
    {'title': 'Education pour Tous', 'subtitle': 'Avenir de la Nation', 'img': 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b'},
    {'title': 'Paix et Securite', 'subtitle': 'Fondement du Developpement', 'img': 'https://images.unsplash.com/photo-1447069387593-a5de0862481e'},
    {'title': 'Culture et Identite', 'subtitle': 'Notre Richesse', 'img': 'https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3'},
    {'title': 'Jeunesse dAvenir', 'subtitle': 'Espoir de la Republique', 'img': 'https://images.unsplash.com/photo-1529390079861-591de354faf5'},
  ];
    ...
    return SliverAppBar(
      ...
      title: Row(
        children: [
          ...
          Container(width: 32, height: 22, decoration: BoxDecoration(borderRadius: BorderRadius.circular(3), color: primaryBlue), child: const Center(child: Text('CD', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)))),
          const SizedBox(width: 8),
          const Expanded(child: Text('REPUBLIQUE DEMOCRATIQUE\nDU CONGO', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: primaryBlue, height: 1.1))),
          ...
          const SizedBox(width: 8),
          InkWell(onTap: () => context.push('/mon-pays/admin'), child: const CircleAvatar(radius: 18, backgroundImage: NetworkImage('https://i.pravatar.cc/100'))),
        ],
      ),
    );
  }
  ...
  Widget _circleIcon(IconData icon, VoidCallback onTap) {
    return InkWell(
      ...
      child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: cardBorder), color: Colors.white), child: Icon(icon, size: 20, color: primaryBlue)),
    );
  }
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: const Row(children: [Text('Espace Citoyen', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 16)), SizedBox(width: 8), Expanded(child: Text('Informer Comprendre Participer', style: TextStyle(color: gold, fontSize: 10, fontWeight: FontWeight.w600)))]),
        ),
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(president.title?? 'President de la Republique', style: const TextStyle(fontSize: 12, color: mutedText, fontWeight: FontWeight.w600)),
                  ])),
                ]),
              ),
              const SizedBox(height: 14),
  ...
  Widget _buildAgencesFull() {
    final items = [
      {'icon': Icons.account_balance, 'label': 'Presidence'},
      {'icon': Icons.flag, 'label': 'Gouvernement'},
      {'icon': Icons.gavel, 'label': 'Parlement'},
      {'icon': Icons.work, 'label': 'Ministeres'},
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [const Text('4. Agences et Institutions', style: TextStyle(fontWeight: FontWeight.w900, color: primaryBlue, fontSize: 16)), const Spacer(), InkWell(onTap: () => _showComingSoon(), child: const Text('Explorer', style: TextStyle(color: Color(0xFF5B8DEF), fontSize: 13)))]),
        const SizedBox(height: 16),
  ...
  Widget _buildALaUneFull() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: cardBorder)),
      child: Column(children: [
        Row(children: [const Text('3. A la Une', style: TextStyle(fontWeight: FontWeight.w900, color: primaryBlue, fontSize: 16)), const Spacer(), InkWell(onTap: () => _showComingSoon(), child: const Text('Voir toutes', style: TextStyle(color: Color(0xFF5B8DEF), fontSize: 13)))]),
        const SizedBox(height: 12),
        SizedBox(height: 165, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: 5, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (context, i) {
          return Container(width: 140, decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: lightBg), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Padding(padding: EdgeInsets.all(8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('27 Mai 2025', style: TextStyle(fontSize: 9, color: mutedText)), SizedBox(height: 2), Text('Inauguration du Pont Marechal', maxLines: 2, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700))]))
          ]));
        }))
      ]),
    );
  ...
  Widget _buildQuickAccess() {
    final items = [
      {'icon': Icons.videocam_rounded, 'label': 'Videos Officielles'},
  ...
  Widget _buildAlertRow() => Row(children: [Expanded(child: _alertCard(rdcRed, 'Personne Recherchee', Icons.warning_amber_rounded)), const SizedBox(width: 12), Expanded(child: _alertCard(primaryBlue, 'Recherche Personnalisee', Icons.search_rounded))]);
  ...
  Widget _buildFiguresHistoriquesBig() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 6),
        const Text('Decouvrez ceux qui ont marque notre histoire.', style: TextStyle(color: mutedText, fontSize: 12)),
        const SizedBox(height: 14),
        SizedBox(height: 180, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: 6, separatorBuilder: (_, __) => const SizedBox(width: 12), itemBuilder: (context, i) { return Container(width: 140, decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), color: lightBg, border: Border.all(color: cardBorder)), child: Column(children: [ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(16)), child: Image.network('https://i.pravatar.cc/200?img=${30 + i}', height: 110, width: 140, fit: BoxFit.cover)), const SizedBox(height: 8), const Text('Patrice Emery', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)), const Text('1960 - Heros National', style: TextStyle(fontSize: 10, color: mutedText))])); }))
      ]),
    );
  }

  Widget _buildCitoyensBanner() => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: primaryBlue, borderRadius: BorderRadius.circular(20)), child: Row(children: [const Icon(Icons.star, color: gold, size: 28), const SizedBox(width: 10), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Citoyens Exemplaires', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), Text('Ils batissent la RDC chaque jour', style: TextStyle(color: Colors.white70, fontSize: 11))])), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: gold, foregroundColor: primaryBlue), onPressed: () => _showComingSoon(), child: const Text('Decouvrir'))]));
