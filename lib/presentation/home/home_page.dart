// lib/app_router.dart - BUILD VERT FINAL - FIX IMPORT CASSE
...
// THIX Money, Media - FIX: Suppression import cassé, on utilise BusHomePage comme hub reservation
...
class NoTransitionPage<T> extends Page<T> {
  ...
  @override Route<T> createRoute(BuildContext context) => PageRouteBuilder<T>(settings: this, transitionDuration: Duration.zero, reverseTransitionDuration: Duration.zero, pageBuilder: (c,a,s) => child, transitionsBuilder: (c,a,s,ch) => ch);
}
    ...
    return GoRouter(
      ...
      redirect: (context, state) {
        ...
        final isPublic = loc == AppRoutes.start || loc == AppRoutes.home || loc == AppRoutes.publicProfile ||
            ...
            loc.startsWith('/thix-reservation') || loc.startsWith('/agency') || loc == AppRoutes.reservation;
        ...
        if (logged && isAuth) return auth.currentUser?.accountType == AccountType.enterprise ? AppRoutes.enterpriseDashboard : AppRoutes.userDashboard;
      ...
      routes: [
        ...
        GoRoute(path: AppRoutes.personalReg, name: 'personalReg', pageBuilder: (_, state) { final step = int.tryParse(state.uri.queryParameters['step'] ?? '') ?? 1; return NoTransitionPage(child: PersonalRegistrationPage(initialStep: step)); }),
        ...
        GoRoute(path: AppRoutes.activationReceipt, name: 'activationReceipt', pageBuilder: (_, state) { final qp = state.uri.queryParameters; return NoTransitionPage(child: ActivationReceiptPage(txRef: qp['txRef'], method: qp['method'], amount: qp['amount'], currency: qp['currency'], paidAt: DateTime.tryParse(qp['paidAt'] ?? ''))); }),
        ...
        GoRoute(path: AppRoutes.enterprise, name: 'enterpriseEntry', redirect: (_, __) { if (!auth.isAuthenticated) return AppRoutes.login; return auth.currentUser?.accountType == AccountType.enterprise ? AppRoutes.enterpriseDashboard : AppRoutes.enterpriseReg; }),
        ...
        GoRoute(path: '${AppRoutes.enterprisePortalBasePath}/:slug', name: 'enterprisePortal', pageBuilder: (_, state) => NoTransitionPage(child: EnterprisePortalPage(companySlug: state.pathParameters['slug']!)), routes: [GoRoute(path: 'dashboard/:section', name: 'enterprisePortalDashboard', pageBuilder: (_, state) => NoTransitionPage(child: EnterpriseDashboardShellPage(companySlug: state.pathParameters['slug']!, section: state.pathParameters['section'] ?? 'overview'))), GoRoute(path: 'dashboard', name: 'enterprisePortalDashboardRoot', redirect: (_, state) => '${AppRoutes.enterprisePortalBase(state.pathParameters['slug']!)}/dashboard/overview'),]),
        ...
        GoRoute(path: AppRoutes.chatConversation, name: 'chat_conversation', pageBuilder: (_, state) { final convId = state.pathParameters['conversationId']!; final conv = state.extra as ChatConversation? ?? ChatConversation(id: convId, isGroup: false, participantIds: [], updatedAt: DateTime.now()); return NoTransitionPage(child: ThixChat.ChatScreen(conversationId: convId, conversation: conv)); }),
        ...
        GoRoute(path: AppRoutes.chatEscalate, name: 'chatEscalate', pageBuilder: (context, state) { final conversationId = state.pathParameters['conversationId']!; final fromAgentId = state.uri.queryParameters['agentId'] ?? ''; final fromAgentName = state.uri.queryParameters['agentName']; return NoTransitionPage(child: EscalateConversationPage(conversationId: conversationId, fromAgentId: fromAgentId, fromAgentName: fromAgentName)); }),
        GoRoute(path: AppRoutes.chatEscalationHandle, name: 'chatEscalationHandle', pageBuilder: (context, state) { final escalationId = state.pathParameters['escalationId']!; final agentId = state.uri.queryParameters['agentId'] ?? ''; return NoTransitionPage(child: HandleEscalationPage(escalationId: escalationId, agentId: agentId)); }),
        GoRoute(path: AppRoutes.chatEscalationHistory, name: 'chatEscalationHistory', pageBuilder: (context, state) { final conversationId = state.pathParameters['conversationId']!; return NoTransitionPage(child: EscalationHistoryPage(conversationId: conversationId)); }),
        GoRoute(path: AppRoutes.chatEscalationDashboard, name: 'chatEscalationDashboard', pageBuilder: (context, state) { final agentId = state.uri.queryParameters['agentId'] ?? ''; final levelIndex = int.tryParse(state.uri.queryParameters['level'] ?? '0') ?? 0; final level = EscalationLevel.values[levelIndex]; return NoTransitionPage(child: EscalationDashboardPage(agentId: agentId, agentLevel: level)); }),
        ...
        GoRoute(path: '${AppRoutes.networkChatBasePath}/:userId', name: 'networkChat', pageBuilder: (_, state) { final uid = state.pathParameters['userId']!; final extra = state.extra; String name = 'Discussion'; String? avatar; if (extra is String) name = extra; else if (extra is Map) { name = extra['userName'] as String? ?? name; avatar = extra['userAvatar'] as String?; } return NoTransitionPage(child: network_chat.ChatScreen(userId: uid, userName: name, userAvatar: avatar)); }),
        ...
        GoRoute(path: '/opportunities/:opportunityId', name: 'opportunityDetails', pageBuilder: (_, state) { final oppId = state.pathParameters['opportunityId'] ?? ''; final applied = (state.uri.queryParameters['applied'] ?? '').trim() == '1'; return NoTransitionPage(child: OpportunityDetailsPage(opportunityId: oppId, applied: applied)); }),
        ...
        GoRoute(path: '/jobs/:jobId', name: 'jobDetails', pageBuilder: (_, state) { final jobId = state.pathParameters['jobId'] ?? ''; final applied = (state.uri.queryParameters['applied'] ?? '').trim() == '1'; return NoTransitionPage(child: JobDetailsPage(jobId: jobId, applied: applied)); }),
        ...
        ...educationRoutes, ...instructorRoutes,
        GoRoute(path: '/moderator', name: 'moderatorHome', builder: (context, state) => const ModeratorHome(), routes: [GoRoute(path: 'events', name: 'moderatorEvents', builder: (context, state) => const ModeratorEventList()), GoRoute(path: 'event/create', name: 'moderatorEventCreate', builder: (context, state) => const ModeratorEventForm()), GoRoute(path: 'event/edit/:id', name: 'moderatorEventEdit', builder: (context, state) => ModeratorEventForm(eventId: state.pathParameters['id']!)),]),
        ...
        GoRoute(path: AppRoutes.thixMediaVideo, name: 'thixMediaVideo', pageBuilder: (_, state) { final title = (state.uri.queryParameters['title'] ?? '').trim(); final url = (state.uri.queryParameters['url'] ?? '').trim(); return NoTransitionPage(child: VideoPlayerPage(title: title.isEmpty ? 'Lecture vidéo' : title, videoUrl: url)); }),

        // FIX FINAL - On utilise BusHomePage comme hub, plus de ThixReservationHomePage manquant
        GoRoute(path: AppRoutes.reservation, name: 'thixreservation', pageBuilder: (_, __) => const NoTransitionPage(child: BusHomePage())),
        ...
        GoRoute(path: AppRoutes.thixEventReservation, name: 'thixEventReservation', pageBuilder: (_, state) { final eventId = state.pathParameters['eventId']!; final quantity = int.tryParse(state.uri.queryParameters['quantity'] ?? '1') ?? 1; return NoTransitionPage(child: EventReservationPage(eventId: eventId, quantity: quantity)); }),
        ...
        GoRoute(path: AppRoutes.thixEventWaitingQueue, name: 'thixEventWaitingQueue', pageBuilder: (_, state) { final eventId = state.pathParameters['eventId']!; final quantity = int.tryParse(state.uri.queryParameters['quantity'] ?? '1') ?? 1; return NoTransitionPage(child: WaitingQueuePage(eventId: eventId, requestedQuantity: quantity)); }),

        // BUS + AGENCY - ACCES GARANTI
        ...
        GoRoute(path: '/thix-reservation/bus/payment', name: 'bus-payment', pageBuilder: (_, state) { final map = state.extra as Map<String, dynamic>; return NoTransitionPage(child: BusPaymentPage(trip: map['trip'] as BusTripModel, seats: (map['seats'] as List).cast<String>())); }),
        ...
        GoRoute(path: AppRoutes.thixMarket, name: 'thixMarket', pageBuilder: (_, __) => NoTransitionPage(child: const MarketHomePage()), routes: [
          ...
          GoRoute(path: 'chat/:shopId', name: 'marketChatSeller', pageBuilder: (_, state) { final shopId = state.pathParameters['shopId']!; final extra = state.extra as Map?; return NoTransitionPage(child: ChatPage(conversationId: '', shopId: shopId, title: extra?['title'] ?? 'Vendeur', avatar: extra?['userAvatar'] as String?)); }),
