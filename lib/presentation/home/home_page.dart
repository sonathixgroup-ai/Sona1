...
class _PinnedHeaderDelegate extends SliverPersistentHeaderDelegate {
  ...
  @override double get maxExtent => _headerExtent();
  @override double get minExtent => _headerExtent();
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: overlapsContent ? [const BoxShadow(color: AppColors.shadowSecondary, blurRadius: 14, offset: Offset(0, 8))] : null,
      ),
  ...
  @override bool shouldRebuild(covariant _PinnedHeaderDelegate oldDelegate) {
    return safeTop != oldDelegate.safeTop || displayName != oldDelegate.displayName || photoUrl != oldDelegate.photoUrl || isAuthenticated != oldDelegate.isAuthenticated;
  }
}
...
class _PremiumHeader extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Padding(padding: EdgeInsets.fromLTRB(AppSpacing.xl, safeTop + 10, AppSpacing.xl, 10), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [const _RotatingGreeting(), Text(displayName, style: const TextStyle(color: AppColors.darkText, fontSize: 13, fontWeight: FontWeight.w800, letterSpacing: -0.2))]), Row(children: [Container(width: 34, height: 34, decoration: BoxDecoration(color: AppColors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.cardBorder, width: 0.5), boxShadow: AppShadows.secondary), child: const Icon(Icons.search_rounded, color: AppColors.darkText, size: 18)), const SizedBox(width: AppSpacing.s), GestureDetector(onTap: () { showModalBottomSheet(context: context, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(28))), builder: (_) => const LanguageSheet()); }, child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: const Color(0xFFE8F0FE), borderRadius: BorderRadius.circular(10)), child: Text(context.watch<LocaleController>().locale.languageCode.toUpperCase(), style: const TextStyle(color: Color(0xFF3A5BA0), fontWeight: FontWeight.bold)))), const SizedBox(width: AppSpacing.s), Material(color: Colors.transparent, shape: const CircleBorder(), clipBehavior: Clip.antiAlias, child: InkWell(onTap: onProfileTap, customBorder: const CircleBorder(), child: Container(width: 34, height: 34, padding: const EdgeInsets.all(2), decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: AppColors.white, width: 2), boxShadow: AppShadows.secondary, color: AppColors.cardBorder), child: ClipOval(child: trimmedPhoto.isNotEmpty ? Image.network(trimmedPhoto, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Image.asset('assets/images/African_businessman_in_suit_grayscale_1775573970767.jpg', fit: BoxFit.cover)) : Image.asset('assets/images/African_businessman_in_suit_grayscale_1775573970767.jpg', fit: BoxFit.cover)))))] )]));
  }
}
...
class _RotatingGreetingState extends State<_RotatingGreeting> {
  static const List<Map<String, String>> _greetings = [{'lang': 'Lingala', 'text': 'Mbote'}, {'lang': 'Kiswahili', 'text': 'Jambo'}, {'lang': 'Tshiluba', 'text': 'Moyo'}, {'lang': 'Kikongo', 'text': 'Mbote'}];
  int _index = 0; Timer? _timer;
  @override void initState() { super.initState(); _timer = Timer.periodic(const Duration(seconds: 3), (_) { if (!mounted) return; setState(() => _index = (_index + 1) % _greetings.length); }); }
...
class _SearchBarOverlayState extends State<_SearchBarOverlay> { @override Widget build(BuildContext context) { return Container(height: 56, padding: const EdgeInsets.symmetric(horizontal: AppSpacing.xl), decoration: BoxDecoration(color: AppColors.white, borderRadius: BorderRadius.circular(AppRadius.searchBar), boxShadow: AppShadows.secondary), child: Row(children: [const Icon(Icons.search_rounded, size: 22, color: AppColors.textSecondary), const SizedBox(width: AppSpacing.s), Expanded(child: TextField(controller: widget.controller, enabled: !widget.isSearching, decoration: const InputDecoration(border: InputBorder.none, hintText: 'Rechercher un THIX ID...', hintStyle: TextStyle(color: AppColors.textSecondary, fontSize: 14)), style: const TextStyle(color: AppColors.darkText, fontSize: 14, fontWeight: FontWeight.w500))), GestureDetector(onTap: widget.isSearching ? null : widget.onVerify, child: Container(padding: const EdgeInsets.symmetric(horizontal: AppSpacing.l, vertical: AppSpacing.s), decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppColors.primaryBlue, AppColors.darkNavy]), borderRadius: BorderRadius.circular(AppRadius.button)), child: const Text('Vérifier', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 12)))), const SizedBox(width: AppSpacing.s), const Icon(Icons.tune_rounded, size: 22, color: AppColors.textSecondary)])); } }
class _PremiumStatusCard extends StatelessWidget { @override Widget build(BuildContext context) { return Container(height: 84, decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [AppColors.premiumSoftStart, AppColors.premiumSoftEnd]), border: Border.all(color: AppColors.cardBorder, width: 0.7), borderRadius: BorderRadius.circular(AppRadius.mainCard), boxShadow: AppShadows.main), padding: const EdgeInsets.symmetric(horizontal: AppSpacing.xl, vertical: AppSpacing.l), child: Row(children: [const Icon(Icons.stars_rounded, color: AppColors.premiumAccent, size: 26), const SizedBox(width: AppSpacing.m), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: const [Text('Membre Premium', style: TextStyle(color: AppColors.darkText, fontSize: 14, fontWeight: FontWeight.w800, letterSpacing: -0.2)), Text('Score de confiance : 98%', style: TextStyle(color: AppColors.textSecondary, fontSize: 12, fontWeight: FontWeight.w600))])), Container(padding: const EdgeInsets.symmetric(horizontal: AppSpacing.m, vertical: AppSpacing.s), decoration: BoxDecoration(color: AppColors.darkText, borderRadius: BorderRadius.circular(AppRadius.button)), child: const Text('Voir', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 12)))])); } }

// ============================================================================
// ✅ CORRECTION ICI SEULEMENT - 4 lignes
// ============================================================================
...
class _QuickActionItem extends StatelessWidget {
  ...
  const _QuickActionItem({required this.icon, required this.label, required this.accent, required this.onTap});
...
class _ServicesConstellation extends StatefulWidget {
  final SectionBadgeCounts counts; final void Function(String key) onServiceTap; final VoidCallback onHomeTap; final VoidCallback onMiniAppsTap; final VoidCallback onDocumentsTap; final VoidCallback onProfileTap; final VoidCallback onScanTap;
  const _ServicesConstellation({required this.counts, required this.onServiceTap, required this.onHomeTap, required this.onMiniAppsTap, required this.onDocumentsTap, required this.onProfileTap, required this.onScanTap});
  @override State<_ServicesConstellation> createState() => _ServicesConstellationState();
}
class _ServicesConstellationState extends State<_ServicesConstellation> with TickerProviderStateMixin {
  ...
  List<_ServiceNodeData> _nodes() { final c = widget.counts; return [_ServiceNodeData(key: 'thixMedia', icon: Icons.play_circle_filled, title: 'THIX MEDIA', color: AppColors.domainMedia), _ServiceNodeData(key: 'thixMarket', icon: Icons.storefront_rounded, title: 'THIX Market', color: AppColors.domainMarket), _ServiceNodeData(key: 'formations', icon: Icons.school_rounded, title: 'Formations', color: AppColors.domainLearning, badge: c.formations), _ServiceNodeData(key: 'emplois', icon: Icons.work_rounded, title: 'Emplois', color: AppColors.domainJobs, badge: c.jobs), _ServiceNodeData(key: 'thixInfo', icon: Icons.newspaper_rounded, title: 'THIX INFO', color: AppColors.domainInfo, badge: c.info), _ServiceNodeData(key: 'opportunites', icon: Icons.lightbulb_rounded, title: 'Opportunités', color: AppColors.domainOpportunity), _ServiceNodeData(key: 'evenements', icon: Icons.event_rounded, title: 'Événements', color: AppColors.domainEvents, badge: c.events), _ServiceNodeData(key: 'reseauPro', icon: Icons.groups_rounded, title: 'Réseau Pro', color: AppColors.domainNetwork), _ServiceNodeData(key: 'thixSante', icon: Icons.local_hospital_rounded, title: 'THIX Santé', color: AppColors.domainHealth), _ServiceNodeData(key: 'thixMoney', icon: Icons.account_balance_wallet_rounded, title: 'Thix Money', color: AppColors.domainMoney), _ServiceNodeData(key: 'monPays', icon: Icons.flag, title: 'Mon Pays', color: AppColors.domainGov), _ServiceNodeData(key: 'reservation', icon: Icons.confirmation_number_rounded, title: 'Réservation', color: AppColors.domainReservation)]; }
