...
// V2 DESIGN MAQUETTE - HERO AUTO-SCROLL VRAIES IMAGES - SUPABASE
import 'dart:async';
...
class _BusHomePageState extends State<BusHomePage> {
  ...
  final List<Map<String, String>> _heroSlides = [
    ...
    },
    {
      ...
      'sub': 'Chauffeurs vérifiés, bus suivis GPS',
      'cta': 'Réserver maintenant',
      'img': 'https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=1200&q=80',
    },
    {
      'title': 'Jusqu\'à -30% sur\nvos trajets',
      'sub': 'Offre limitée cette semaine\nGoma - Bukavu',
      'cta': 'Profiter',
      'img': 'https://images.unsplash.com/photo-1520105077905-d6a4ef6f1c32?w=1200&q=80',
    },
  ];
  ...
  Future<void> _init() async {
    ...
    if (user!= null) {
      try {
        ...
        try {
          final agency2 = await Supabase.instance.client.from('bus_agencies').select('id').eq('user_id', user!.id).maybeSingle();
          if (agency2!= null && mounted) setState(() => _hasAgency = true);
        } catch (_) {}
      }
    }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(children: [
          ...
          const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('Réservez simplement, voyagez sereinement.', style: TextStyle(fontSize: 10.5, color: Color(0xFF64748B), fontWeight: FontWeight.w500)),
          ]),
        ]),
        actions: [
          // ICONES AGENCE EN HAUT (plus de gros bouton)
          IconButton(
          ...
          IconButton(onPressed: ()=> context.push('/notifications'), icon: Badge(label: const Text('3'), child: Container(padding: const EdgeInsets.all(7), decoration: BoxDecoration(color: const Color(0xFFF1F5F9), shape: BoxShape.circle), child: const Icon(Icons.notifications_none_rounded, size: 20)))),
          Padding(padding: const EdgeInsets.only(right: 10), child: CircleAvatar(radius: 18, backgroundColor: const Color(0xFFD6E4FF), backgroundImage: Supabase.instance.client.auth.currentUser?.userMetadata?['avatar_url']!= null? NetworkImage(Supabase.instance.client.auth.currentUser!.userMetadata!['avatar_url']) : null, child: Supabase.instance.client.auth.currentUser?.userMetadata?['avatar_url'] == null? const Icon(Icons.person, color: kPrimary) : null)),
        ],
      ),
      body: RefreshIndicator(onRefresh: _init, child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(12, 8, 12, 90), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // HERO AUTO-SCROLL VRAIES IMAGES BUS
          ...
          PageView.builder(
            ...
            itemBuilder: (_, i){
              ...
              return ClipRRect(borderRadius: BorderRadius.circular(18), child: Stack(children: [
                Positioned.fill(child: Image.network(s['img']!, fit: BoxFit.cover, errorBuilder: (_,__,___)=> Container(color: kPrimary))),
        ...
        // CATEGORIES COMME MAQUETTE
        Container(padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 6), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 8)]), child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          _Cat(icon: Icons.apartment_rounded, label: 'Hôtels', color: const Color(0xFF0B4FE3), onTap: ()=> context.push('/thix-reservation/hotels')),
          _Cat(icon: Icons.flight_rounded, label: 'Vols', color: const Color(0xFF0E8A5B), onTap: ()=> context.push('/thix-reservation/flights')),
          _Cat(icon: Icons.directions_bus_filled_rounded, label: 'Bus', color: kPrimary, active: true, onTap: (){}),
          _Cat(icon: Icons.local_taxi_rounded, label: 'Transports', color: const Color(0xFFE67E22), onTap: ()=> context.push('/thix-reservation/taxi')),
          _Cat(icon: Icons.local_activity_rounded, label: 'Événements', color: const Color(0xFFDB2777), onTap: ()=> context.push('/thix-event')),
          _Cat(icon: Icons.restaurant_rounded, label: 'Restaurants', color: const Color(0xFFEA580C), onTap: ()=> context.push('/thix-reservation/restaurants')),
          _Cat(icon: Icons.home_work_rounded, label: 'Locations', color: const Color(0xFF0E8A5B), onTap: ()=> context.push('/thix-reservation/hotels')),
          _Cat(icon: Icons.more_horiz_rounded, label: 'Plus', color: const Color(0xFF64748B), onTap: (){}),
        ])),
        const SizedBox(height: 14),

        // RESERVATION RAPIDE - DESIGN MAQUETTE
        Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 10)]), child: Column(children: [
          Row(children: [
            const Text('Réservation rapide de bus', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13.5)),
            const Spacer(),
            InkWell(onTap: (){}, child: const Row(children: [Text('Voir tout', style: TextStyle(fontSize: 11, color: kPrimary, fontWeight: FontWeight.w700)), Icon(Icons.chevron_right_rounded, size: 14, color: kPrimary)])),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _FieldBox(icon: Icons.my_location_rounded, label: 'Départ', value: searchP.departure?? 'Abidjan', onTap: ()=> _showCityPicker(context, true))),
            Container(margin: const EdgeInsets.symmetric(horizontal: 6), padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: const Color(0xFFF1F5F9), shape: BoxShape.circle, border: Border.all(color: const Color(0xFFE2E8F0))), child: const Icon(Icons.swap_horiz_rounded, size: 16, color: Color(0xFF64748B))),
            Expanded(child: _FieldBox(icon: Icons.location_on_rounded, label: 'Arrivée', value: searchP.arrival?? 'Yamoussoukro', onTap: ()=> _showCityPicker(context, false))),
          ]),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: _FieldBox(icon: Icons.calendar_today_rounded, label: 'Date de départ', value: searchP.departureDate!= null? '${searchP.departureDate!.day} ${_month(searchP.departureDate!.month)} ${searchP.departureDate!.year}' : '18 Mai 2024', onTap: ()=> _pickDate(context))),
            ...
            const SizedBox(width: 10),
            SizedBox(height: 48, child: ElevatedButton.icon(onPressed: () async { await searchP.search(); if(context.mounted) context.push('/thix-reservation/bus/search'); }, icon: const Icon(Icons.search_rounded, size: 18), label: const Text('Rechercher un bus', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)), style: ElevatedButton.styleFrom(backgroundColor: kPrimary, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0))),
          ]),
        ])),
        const SizedBox(height: 16),

        // ROUTES POPULAIRES - SUPABASE
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            FutureBuilder(
              ...
              builder: (_, snap){
                final list = (snap.data as List?)?? [
                  {'label':'Sièges\nconfortables','icon_name':'seat'},
                  {'label':'Wi-Fi\ngratuit','icon_name':'wifi'},
                  {'label':'Climatisation','icon_name':'ac'},
                  {'label':'Bagages\nautorisés','icon_name':'luggage'},
          ...
          Expanded(flex: 2, child: Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFEEF4FF), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFD6E4FF))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.headset_mic_rounded, color: kPrimary, size: 20)), const SizedBox(width: 8), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Besoin d\'aide?', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 12)), Text('Notre équipe est disponible 24h/24 et 7j/7', style: TextStyle(fontSize: 10, color: Color(0xFF64748B), height: 1.2))]))]),
            const SizedBox(height: 12),
      ...
      bottomNavigationBar: Container(decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 16)]), child: SafeArea(child: BottomNavigationBar(
        currentIndex: 0, selectedItemColor: kPrimary, unselectedItemColor: const Color(0xFF94A3B8), type: BottomNavigationBarType.fixed, backgroundColor: Colors.white, elevation: 0,
        items: [
          ...
          BottomNavigationBarItem(icon: Container(padding: const EdgeInsets.all(14), decoration: const BoxDecoration(color: kPrimary, shape: BoxShape.circle), child: const Icon(Icons.add_rounded, color: Colors.white, size: 22)), label: 'Réserver'),
  ...
  String _month(int m){ const ms=['','Jan','Fév','Mar','Avr','Mai','Juin','Juil','Août','Sep','Oct','Nov','Déc']; return ms[m]; }
