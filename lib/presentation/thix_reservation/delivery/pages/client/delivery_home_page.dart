...
// SANS MOCK - FORMAT CLEAN - BUILD VERT
...
class _DeliveryHomePageState extends State<DeliveryHomePage> {
        ...
        children: [
          ...
          const Column(
            ...
            children: [
              ...
              Text(
                "Livrez vos colis en toute simplicité.",
                style: TextStyle(
                  fontSize: 7.5,
  ...
  Widget _buildHero(DeliveryClientProvider prov) {
    final name = prov.userName.isEmpty? "Michel" : prov.userName;
    return Container(
      height: 138,
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(16),
        gradient: const LinearGradient(
          colors: [Color(0xFF5B2BD6), Color(0xFF7C4DFF)],
        ),
      ),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(
              width: 160,
              child: Text(
                "Envoyez ou recevez vos colis en toute simplicité",
                style: TextStyle(
                  color: Colors.white,
                  fontSize: 13,
                  fontWeight: FontWeight.w800,
      ...
      child: Row(
        ...
        children: cats.map((c) {
          ...
          return InkWell(
            onTap: () {
              setState(() {
                _selectedCat = c["label"]!;
              });
              if (c["route"]!= AppRoutes.deliveryHome) {
          ...
          Row(
            children: [
              _toggleBtn(
                "National",
                prov.isNational,
                () => prov.setNational(true),
              ),
              const SizedBox(width: 6),
              _toggleBtn(
                "International",
               !prov.isNational,
                () => prov.setNational(false),
              ),
            ],
          ),
          const SizedBox(height: 8),
          Row(
            children: [
              ...
              Expanded(
                child: _fieldBox(
                  "Destinataire",
                  prov.toCity.isEmpty? "Choisir ville" : prov.toCity,
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFF5B2BD6),
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(8),
                ),
                elevation: 0,
              ),
              onPressed: prov.isCalculating
                 ? null
                  : () {
                      if (prov.fromCity.isEmpty ||
                          prov.toCity.isEmpty) {
              ...
              child: Text(
                prov.calculatedPrice > 0
                   ? "${prov.calculatedPrice} FCFA - Continuer"
                    : "Calculer le prix et continuer",
                style: const TextStyle(
                  color: Colors.white,
                  fontSize: 9.5,
                  fontWeight: FontWeight.w700,
  ...
  Widget _toggleBtn(String label, bool sel, VoidCallback tap) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: sel? const Color(0xFFF0EBFF) : Colors.white,
          borderRadius: BorderRadius.circular(7),
          border: Border.all(
            color: sel
               ? const Color(0xFF5B2BD6)
  ...
  Widget _fieldBox(String head, String value, VoidCallback tap) {
    return InkWell(
      onTap: tap,
      child: Container(
  ...
  Widget _buildOffresReelles(DeliveryClientProvider prov) {
    if (prov.offers.isEmpty) {
      return Container(
        ...
        child: const Text(
          "Aucune offre disponible - Admin doit créer dans delivery_offers",
          style: TextStyle(
            fontSize: 9,
            color: Color(0xFF8B8BA3),
          ),
  ...
  Widget _buildHowItWorks() {
    return Container(
      ...
      child: const Text(
        "Comment ça marche? 1. Détails → 2. Mode → 3. Paiement → 4. Livraison",
        style: TextStyle(
          fontSize: 8.5,
          color: Color(0xFF8B8BA3),
        ),
  ...
  void _pickCity(bool isFrom, DeliveryClientProvider prov) {
    final cities =
        prov.popularRoutes.map((e) => e.fromCity).toSet().toList();
    if (cities.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Aucune route - Admin doit créer delivery_routes"),
        ),
      );
      return;
    }
    showModalBottomSheet(
      ...
      builder: (_) => ListView.builder(
        ...
        itemBuilder: (_, i) => ListTile(
          ...
          onTap: () {
            if (isFrom) {
              prov.setFromCity(cities[i]);
            } else {
              prov.setToCity(cities[i]);
            }
