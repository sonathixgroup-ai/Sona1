...
class JobApplyPage extends StatefulWidget {
  ...
  @override State<JobApplyPage> createState() => _JobApplyPageState();
}
...
class _JobApplyPageState extends State<JobApplyPage> {
  ...
  @override void dispose() {
    _thixCtrl.dispose(); _messageCtrl.dispose(); _portfolioCtrl.dispose(); super.dispose();
  }

  @override void didChangeDependencies() {
    super.didChangeDependencies();
    final thixId = context.read<AuthController>().currentUser?.thixId ?? '';
    if (_thixCtrl.text.trim().isEmpty && thixId.isNotEmpty) _thixCtrl.text = thixId;
  }
  ...
  Future<void> _submit() async {
    ...
    try{
      ...
      if(canonical==null || !ThixIdService.isValid(canonical)){
        setState(()=> _error='THIX ID invalide. Ex: ${ThixIdService.exampleV2}'); return;
      }
      final profile = await _profileService.fetchPublicProfileByThixId(canonical);
      if(profile==null){ setState(()=> _error='Aucun profil trouvé pour ce THIX ID.'); return; }

      final uid = context.read<AuthController>().currentUser?.id;
      String? resumeUrl; String? videoUrl; final diplomaUrls=<String>[];

      if(uid!=null && uid.isNotEmpty){
        // Uploads en parallèle + validation
      ...
      await _jobService.submitApplication(
        jobId: widget.jobId, applicantThixId: canonical,
        message: _messageCtrl.text, portfolioUrl: _portfolioCtrl.text.trim().isEmpty? null : _portfolioCtrl.text.trim(),
        videoIntroUrl: videoUrl, resumeUrl: resumeUrl, diplomaUrls: diplomaUrls,
      );
      if(!mounted) return;
      context.go('/jobs/${widget.jobId}?applied=1');
    } catch(e){
      debugPrint('JobApply apply failed $e');
      if(mounted) setState(()=> _error='Erreur lors de la candidature. Réessaie.');
    } finally { if(mounted) setState(()=> _loading=false); }
  }

  // MILLION-READY UPLOAD
  Future<String?> _tryUpload({required String uid, required fp.PlatformFile? file, required String kind, required int maxMB, required List<String> allowed}) async {
    ...
    try{
      if(file.size > maxMB*1024*1024){ setState(()=> _error='${file.name} trop lourd >${maxMB}MB'); return null; }
      final ext = (file.extension??'').toLowerCase();
      if(!allowed.contains(ext)){ setState(()=> _error='${file.name} type non autorisé'); return null; }
      ...
      return null; // Ne jamais exposer en publicUrl
    } catch(e){
      debugPrint('upload failed $e'); return null;
    }
  }
  ...
  Widget build(BuildContext context){
    ...
    return Scaffold(
      ...
      body: SafeArea(child: FutureBuilder(future: _jobService.fetchJob(widget.jobId), builder: (context,snap){
        final job = snap.data;
        if(snap.connectionState!=ConnectionState.done) return const Center(child: CircularProgressIndicator(color: LearningCyberColors.neonCyan));
        if(job==null) return Padding(padding: const EdgeInsets.all(AppSpacing.lg), child: Column(children: [_TopBar(jobId: widget.jobId), const Spacer(), Text('Offre introuvable.', style: context.textStyles.titleMedium?.copyWith(color: LearningCyberColors.text)), const SizedBox(height: AppSpacing.lg), SizedBox(width: double.infinity, child: FilledButton(onPressed: ()=>context.go(AppRoutes.jobs), child: const Text('Retour'))), const Spacer()]));
        return SingleChildScrollView(padding: const EdgeInsets.all(AppSpacing.lg), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          _TopBar(jobId: widget.jobId), const SizedBox(height: AppSpacing.md),
          ...
          Container(padding: const EdgeInsets.all(AppSpacing.lg), decoration: BoxDecoration(color: LearningCyberColors.panel.withValues(alpha: 0.72), borderRadius: BorderRadius.circular(AppRadius.xl), border: Border.all(color: LearningCyberColors.stroke.withValues(alpha: 0.9), width: 1.2)), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            Text(me?.hasRealThixId==true? 'Nous avons pré-rempli ton THIX ID.' : 'Entre ton THIX ID (vérifié en base).', style: context.textStyles.bodyMedium?.copyWith(color: LearningCyberColors.textDim, height: 1.55)),
            const SizedBox(height: AppSpacing.md),
            TextField(controller: _thixCtrl, textCapitalization: TextCapitalization.characters, decoration: InputDecoration(labelText: 'THIX ID', hintText: ThixIdService.exampleV2, prefixIcon: const Icon(Icons.badge_rounded), filled: true, fillColor: LearningCyberColors.panelHi.withValues(alpha: 0.78)), onChanged: (_){ if(_error!=null) setState(()=>_error=null); }),
            const SizedBox(height: AppSpacing.md),
            ...
            const SizedBox(height: AppSpacing.lg),
            Text('Attachments', style: context.textStyles.titleSmall?.copyWith(color: LearningCyberColors.text, fontWeight: FontWeight.w900)), const SizedBox(height: 10),
