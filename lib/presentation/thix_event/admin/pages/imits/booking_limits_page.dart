...
class BookingLimitsPage extends StatefulWidget {
  ...
  @override State<BookingLimitsPage> createState() => _BookingLimitsPageState();
}
...
class _BookingLimitsPageState extends State<BookingLimitsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(padding: const EdgeInsets.all(16), child: ElevatedButton(
        ...
        child: _isSaving? const CircularProgressIndicator(color: Colors.white) : const Text('ENREGISTRER LIMITES', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900)),
      )),
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFE7EEFC))), child: Column(children: [
          ...
          const SizedBox(height: 12),
          SwitchListTile(value: _requireId, onChanged: (v)=> setState(()=> _requireId=v), title: const Text('Vérification ID requise', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700)), subtitle: const Text('Pour concerts à forte demande', style: TextStyle(fontSize: 11))),
        ])),
        const SizedBox(height: 16),
  ...
  InputDecoration _deco(String l)=> InputDecoration(labelText: l, filled: true, fillColor: Colors.white, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFFE7EEFC))));
}
