...
class _SeatMapAdminPageState extends State<SeatMapAdminPage> {
  ...
  void initState() {
    ...
    for (int i = 0; i < 26; i++) {
      ...
      if (i < 2) {
        _categoryByRow[letter] = SeatCategory.vip;
      } else if (i < 4) {
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            TextFormField(
              ...
              decoration: _deco('Prix de base FC'), 
              keyboardType: TextInputType.number, 
              onChanged: (v) => _basePrice = double.tryParse(v) ?? 5000
            ),
            const SizedBox(height: 12),
            ...
            Wrap(spacing: 8, runSpacing: 8, children: List.generate(_rows, (i) {
              final letter = String.fromCharCode(65 + i);
              return ChoiceChip(
                label: Text('$letter: ${_categoryByRow[letter]!.name}', style: const TextStyle(fontSize: 10)),
                selected: true,
                onSelected: (_) {
                  setState(() {
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isGenerating 
                  ...
                  : Text('GÉNÉRER ${_rows * _perRow} SIÈGES (BATCH)', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 12)),
              )
            ),
        ])),
  ...
  Widget _buildSeatMap() {
    ...
    return Container(
      ...
      child: Column(children: [
        Container(width: 120, height: 8, decoration: BoxDecoration(color: const Color(0xFF0A1F44), borderRadius: BorderRadius.circular(10))),
        ...
        ...byRow.entries.map((e) => Row(children: [
          SizedBox(width: 20, child: Text(e.key, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800))),
          Expanded(child: Wrap(
            spacing: 4, 
            runSpacing: 4, 
            // FIX : Ajout de <Widget> pour forcer le typage de la liste
            children: e.value.map<Widget>((seat) => Container(
              width: 22, height: 22,
              ...
              child: Center(child: Text('${seat.number}', style: const TextStyle(fontSize: 7, color: Colors.white, fontWeight: FontWeight.w700))),
            )).toList()
          )),
        ])),
    ]));
  ...
  Color _colorForStatus(EventSeat s) {
    ...
    switch(s.category) {
      case SeatCategory.vip: return const Color(0xFFFFD700);
      case SeatCategory.gold: return const Color(0xFFD4AF37);
      case SeatCategory.family: return const Color(0xFF2196F3);
  ...
  Widget _buildLegend() => Wrap(spacing: 12, children: [
    ...
    _legendDot(const Color(0xFFFFD700), 'VIP'),
  ]);
