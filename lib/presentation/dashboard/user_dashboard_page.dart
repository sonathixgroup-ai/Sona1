  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: FutureBuilder<AppUser?>(
        future: authController.currentUser, // ✅ Utiliser currentUser au lieu de getCurrentUser
        builder: (context, authSnap) {
          ...
          return FutureBuilder<ThixProfile?>(
            future: profileService.fetchProfile(authUser.id), // ✅ fetchProfile au lieu de fetchMyProfile
            builder: (context, profileSnap) {
              // ✅ Création manuelle d'un profil vide
              final profile = profileSnap.data ?? ThixProfile(
                userId: authUser.id,
                thixId: '',
                fullName: '',
      ...
      child: Column(
        ...
        children: [
          ...
          if (profile.bio.isNotEmpty) ...[
            Text('Bio', style: context.textStyles.titleMedium?.copyWith(color: Colors.white)),
            const SizedBox(height: 8),
            Text(profile.bio, style: context.textStyles.bodyMedium?.copyWith(color: Colors.white70)),
            const SizedBox(height: 16),
          ],
          
          if (profile.website.isNotEmpty) ...[
            Text('Site web', style: context.textStyles.titleMedium?.copyWith(color: Colors.white)),
            const SizedBox(height: 8),
            Text(profile.website, style: context.textStyles.bodyMedium?.copyWith(color: Colors.white70)),
            const SizedBox(height: 16),
          ],
          
          if (profile.location.isNotEmpty) ...[
            Text('Localisation', style: context.textStyles.titleMedium?.copyWith(color: Colors.white)),
            const SizedBox(height: 8),
            Text(profile.location, style: context.textStyles.bodyMedium?.copyWith(color: Colors.white70)),
            const SizedBox(height: 16),
          ],
